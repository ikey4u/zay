...
const emptyForm: ConfigPayload = {
  proxy: {
    enabled: false, subscriptions: [], active_nodes: [], gateway: false, mixed_port: 7890,
...
export default function App() {
  ...
  const persistConfig = async (submitted: ConfigPayload, restartAfter = false) => {
    ...
    try {
      ...
      if (restartAfter && running) {
        ...
        setNotice({ kind: "ok", text: "配置已保存并应用到网络核心。" })
      } else {
        setRestartRequired(running)
        setNotice({ kind: "ok", text: running ? "配置已保存，重启核心后生效。" : "配置已保存，下次启动核心时生效。" })
      }
      await refresh(true)
      return true
  ...
  const toggleProcessTraffic = async (enabled: boolean) => {
    ...
    try {
      ...
      setNotice({ kind: "ok", text: enabled ? "已开始记录按进程流量；关闭或重启核心时会清空。" : "已停止记录并清空按进程流量。" })
    } catch (error) {
  ...
  const coreAction = async (action: "start" | "stop" | "restart") => {
    ...
    try {
      ...
      if (health === "failed") setNotice({ kind: "error", text: next.core.error ?? "核心启动失败" })
      else if (health === "degraded") setNotice({ kind: "warning", text: next.core.error ?? "核心仅部分可用" })
      else setNotice({ kind: "ok", text: action === "stop" ? "核心已停止" : action === "start" ? "核心已启动" : "核心已重启" })
    } catch (error) {
  ...
  const exitWebui = async () => {
    if (!window.confirm("退出 WebUI 会同时停止当前网络核心，确定继续吗？")) return
    setBusy(true)
    try {
...
function ActionMenu({ running, loading, busy, onClose, onRefresh, onCoreAction, onAbout, onExit }: { running: boolean; loading: boolean; busy: boolean; onClose: () => void; onRefresh: () => void; onCoreAction: (action: "start" | "stop" | "restart") => void; onAbout: () => void; onExit: () => void }) {
  ...
  return <><button className="fixed inset-0 z-40 cursor-default bg-black/10" aria-label="关闭 Zay 菜单" onClick={onClose} /><div className="fixed bottom-4 left-4 z-50 w-60 rounded-xl border border-border bg-card/95 p-2 shadow-2xl backdrop-blur-xl lg:left-[60px]">
    <div className="mb-1 px-3 py-2"><div className="text-sm font-semibold">Zay</div><div className="text-[11px] text-muted-foreground">网络核心与 WebUI</div></div>
    <MenuAction icon={RefreshCw} label="刷新状态" disabled={loading} spin={loading} onClick={() => run(onRefresh)} />
    <MenuAction icon={running ? CircleStop : Play} label={running ? "停止核心" : "启动核心"} danger={running} disabled={busy} onClick={() => run(() => onCoreAction(running ? "stop" : "start"))} />
    <MenuAction icon={RotateCw} label="重启核心" disabled={busy || !running} onClick={() => run(() => onCoreAction("restart"))} />
...
function ProxyWorkspace({ form, update, nodes, ruleSets, busy, onTestNodes, onApplyProvider, onSectionChange, onRulesChanged }: { form: ConfigPayload; update: <K extends keyof ConfigPayload["proxy"]>(key: K, value: ConfigPayload["proxy"][K]) => void; nodes: ProxyNode[]; ruleSets: { builtin: RuleSetEntry[]; external: RuleSetEntry[] }; busy: boolean; onTestNodes: (nodes: string[]) => void; onApplyProvider: () => void; onSectionChange: () => void; onRulesChanged: () => void }) { const [section, setSection] = useState<ProxySection>("providers"); return <Workspace title="代理" description="订阅、节点池、规则与系统流量设置" icon={ShieldCheck} nav={[{ id: "providers", label: "代理商", icon: Users, count: form.proxy.subscriptions.length }, { id: "nodes", label: "节点列表", icon: List, count: nodes.length }, { id: "rules", label: "规则集", icon: FileJson, count: ruleSets.builtin.length + ruleSets.external.length + form.proxy.domain_rules.length }, { id: "settings", label: "设置", icon: Settings2 }]} section={section} setSection={(value) => setSection(value as ProxySection)} onSectionChange={onSectionChange}>{section === "providers" ? <Providers form={form} update={update} nodes={nodes} busy={busy} onApply={onApplyProvider} /> : section === "nodes" ? <ProxyNodes form={form} update={update} nodes={nodes} busy={busy} onTestNodes={onTestNodes} /> : section === "rules" ? <RulesPanel form={form} update={update} ruleSets={ruleSets} onRulesChanged={onRulesChanged} /> : <ProxySettings form={form} update={update} />}</Workspace> }
function Providers({ form, update, nodes, busy, onApply }: { form: ConfigPayload; update: <K extends keyof ConfigPayload["proxy"]>(key: K, value: ConfigPayload["proxy"][K]) => void; nodes: ProxyNode[]; busy: boolean; onApply: () => void }) {
...
function RuleFileGroup({ title, description, group, entries, onBack, onChanged }: { title: string; description: string; group: "builtin" | "external"; entries: RuleSetEntry[]; onBack: () => void; onChanged: () => void }) {
  ...
  const saveText = async () => { if (!selected) return; setBusy(true); setMessage(""); try { await uploadRuleSet(`/api/v1/rules/external/${encodeURIComponent(selected.name)}`, draft); setMessage("规则集已保存，重启核心后生效。"); onChanged() } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) } }
  const upload = async (file: File, name?: string) => { setBusy(true); setMessage(""); try { const target = name ?? file.name; await uploadRuleSet(`/api/v1/rules/external/${encodeURIComponent(target)}`, file); setMessage("规则文件已替换，重启核心后生效。"); onChanged(); if (selected) await open({ ...selected, name: target }) } catch (error) { setMessage(error instanceof Error ? error.message : String(error)) } finally { setBusy(false) } }
