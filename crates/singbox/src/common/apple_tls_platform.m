...
box_apple_tls_client_t *singbox_apple_tls_client_create_with_der_anchors(
	int connected_socket,
	const char *server_name,
	const char *alpn,
	size_t alpn_len,
	...
	@autoreleasepool {
		NSMutableArray *anchors = [NSMutableArray arrayWithCapacity:anchor_certificate_count];
		for (size_t index = 0; index < anchor_certificate_count; index++) {
			NSData *data = [NSData dataWithBytes:anchor_certificates[index]
											 length:anchor_certificate_lengths[index]];
			SecCertificateRef certificate = SecCertificateCreateWithData(NULL, (__bridge CFDataRef)data);
			if (certificate == NULL) {
				close(connected_socket);
				box_set_error_message(error_out, "apple TLS: parse trust anchor");
				return NULL;
			}
			[anchors addObject:(__bridge id)certificate];
			CFRelease(certificate);
		}
		return box_apple_tls_client_create(
			connected_socket,
			server_name,
			alpn,
			alpn_len,
			...
			max_version,
			insecure,
			anchor_certificate_count > 0 ? (__bridge void *)anchors : NULL,
			anchor_only,
			has_verify_time,
...
// Rust uses the synchronous receive wrapper. Keep the upstream async entry
// point linkable and release an accidentally delivered result defensively.
void box_apple_tls_read_callback(uintptr_t callback_handle, box_apple_tls_read_result_t *result) {
	(void)callback_handle;
	box_apple_tls_read_result_free(result);
}
