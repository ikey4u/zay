...
impl Device for BufferDevice {
    ...
    fn capabilities(&self) -> DeviceCapabilities {
        let mut capabilities = self.caps.clone();
        capabilities.max_transmission_unit = self.mtu.load(Ordering::Acquire);
        capabilities
    }
}
...
impl BufferDevice {
    pub(crate) fn new(caps: DeviceCapabilities) -> BufferDevice {
        let max_burst_size =
            caps.max_burst_size.unwrap_or(DEFAULT_MAX_BURST_SIZE);
        let mtu = Arc::new(AtomicUsize::new(caps.max_transmission_unit));
        BufferDevice {
            caps,
            mtu,
            max_burst_size,
    ...
    pub(crate) fn mtu_handle(&self) -> Arc<AtomicUsize> {
        self.mtu.clone()
    }
...
mod tests {
    ...
    #[test]
    fn buffer_device_capabilities_follow_live_mtu() {
        let mut capabilities = DeviceCapabilities::default();
        capabilities.max_transmission_unit = 1_406;
        let device = BufferDevice::new(capabilities);
        let mtu = device.mtu_handle();

        assert_eq!(Device::capabilities(&device).max_transmission_unit, 1_406);
        mtu.store(1_280, Ordering::Release);
        assert_eq!(Device::capabilities(&device).max_transmission_unit, 1_280);
    }
}
