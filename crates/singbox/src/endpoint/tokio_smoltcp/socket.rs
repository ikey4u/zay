...
impl IcmpSocket {
    pub(super) async fn new(
        ...
        let handle = reactor.socket_allocator().new_icmp_socket();
        reactor
            .get_socket::<icmp::Socket>(*handle)
    ...
    pub fn set_hop_limit(&self, hop_limit: u8) {
        self.reactor
            .get_socket::<icmp::Socket>(*self.handle)
            .set_hop_limit(Some(hop_limit.max(1)));
    }
    ...
    pub async fn recv_from(
        ...
        poll_fn(|context| {
            let mut socket =
                self.reactor.get_socket::<icmp::Socket>(*self.handle);
            match socket.recv_slice(packet) {
