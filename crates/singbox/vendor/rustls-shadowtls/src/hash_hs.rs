    ...
    pub(crate) fn add_message(&mut self, m: &Message<'_>) {
        match &m.payload {
            MessagePayload::Handshake { encoded, .. } => {
                self.add_raw(encoded.bytes())
            }
            MessagePayload::HandshakeFlight(payload) => {
                self.add_raw(payload.bytes())
            }
            _ => {}
        };
    }
...
mod tests {
    ...
    fn hashes_message_types() {
        ...
        let app_data_ignored = Message {
            ...
            payload: MessagePayload::ApplicationData(Payload::Borrowed(
                b"hello",
            )),
        };
        ...
        let end_of_early_data_flight = Message {
            version: ProtocolVersion::TLSv1_3,
            payload: MessagePayload::HandshakeFlight(Payload::Borrowed(
                b"\x05\x00\x00\x00",
            )),
        };
