...
use crate::msgs::handshake::{
    ClientExtensions, ClientHelloPayload, HandshakeMessagePayload,
    HandshakePayload, KeyShareEntry, Random, ServerNamePayload, SessionId,
    SupportedProtocolVersions,
};
use crate::msgs::message::{Message, MessagePayload};
use crate::{
    CommonState, Error, PeerIncompatible, PeerMisbehaved, ProtocolVersion, Side,
};
...
mod tests {
    ...
    use crate::crypto::{
        ActiveKeyExchange, CryptoProvider, KeyExchangeAlgorithm,
        SupportedKxGroup,
    };
    ...
    use crate::sync::Arc;
    use crate::{
        version, CipherSuiteCommon, SupportedCipherSuite, Tls12CipherSuite,
    };
    ...
    fn test_server_requiring_rpk_client_rejects_x509_client() {
        let mut ch = minimal_client_hello();
        ch.extensions.client_certificate_types =
        ...
        let mut conn =
            ServerConnection::new(server_config_for_rpk().into()).unwrap();
        conn.read_tls(&mut ch.into_wire_bytes().as_slice()).unwrap();
        assert_eq!(
    ...
    fn test_rpk_only_server_rejects_x509_only_client() {
        let mut ch = minimal_client_hello();
        ch.extensions.server_certificate_types =
            Some(vec![CertificateType::X509]);
        let ch = Message {
            version: ProtocolVersion::TLSv1_3,
            payload: MessagePayload::handshake(HandshakeMessagePayload(
    ...
    fn server_config_for_rpk() -> ServerConfig {
        ...
        ServerConfig::builder_with_provider(x25519_provider.into())
            ...
            .with_cert_resolver(Arc::new(
                AlwaysResolvesServerRawPublicKeys::new(Arc::new(
    ...
    fn server_cert() -> Vec<CertificateDer<'static>> {
        vec![
            CertificateDer::from(
                &include_bytes!("../../../test-ca/rsa-2048/end.der")[..],
            ),
            CertificateDer::from(
                &include_bytes!("../../../test-ca/rsa-2048/inter.der")[..],
            ),
        ]
    }
