...
use crate::common_state::{
    CommonState, HandshakeFlightTls12, HandshakeKind, Side, State,
};
...
use crate::msgs::handshake::{
    CertificateChain, ClientKeyExchangeParams, HandshakeMessagePayload,
    HandshakePayload, NewSessionTicketPayload, NewSessionTicketPayloadTls13,
    SessionId,
};
...
mod client_hello {
    ...
    use crate::msgs::handshake::{
        ...
        ClientSessionTicket, Random, ServerExtensionsInput, ServerHelloPayload,
    ...
    impl CompleteClientHelloHandling {
        pub(in crate::server) fn handle_client_hello(
            ...
            let supported_ec_point_formats =
                client_hello.ec_point_formats.unwrap_or_default();
            ...
            if tls13_enabled {
                self.randoms.server[24..]
                    .copy_from_slice(&tls12::DOWNGRADE_SENTINEL);
            }
            ...
            if !self.config.session_storage.can_cache() {
                ...
                self.session_id =
                    SessionId::random(self.config.provider.secure_random)?;
            }
            ...
            let doing_client_auth =
                emit_certificate_req(&mut flight, &self.config)?;
    ...
    fn emit_server_hello(
        ...
        let sh = HandshakeMessagePayload(HandshakePayload::ServerHello(
            ServerHelloPayload {
    ...
    fn emit_server_kx(
        ...
        let signer =
            signing_key.choose_scheme(&sigschemes).ok_or_else(|| {
                Error::General("incompatible signing key".to_string())
            })?;
    ...
    fn emit_certificate_req(
        ...
        let creq =
            HandshakeMessagePayload(HandshakePayload::CertificateRequest(cr));
        ...
        let client_cert = match cert_chain.split_first() {
            ...
            Some((end_entity, intermediates)) => {
                ...
                self.config
                    ...
                    .map_err(|err| {
                        cx.common.send_cert_verify_error_alert(err)
                    })?;
            ...
            match self.transcript.take_handshake_buf() {
                ...
                None => {
                    ...
                    return Err(cx.common.send_fatal_alert(
                        ...
                        Error::General(
                            "client authentication not set up".into(),
                        ),
                    ));
                }
            }
...
fn emit_ticket(
    ...
    let plain = get_server_connection_value_tls12(secrets, using_ems, cx, now)
        .get_encoding();
...
fn emit_finished(
    ...
    let f = Message {
        version: ProtocolVersion::TLSv1_2,
        payload: MessagePayload::handshake(HandshakeMessagePayload(
            HandshakePayload::Finished(verify_data_payload),
        )),
    };
...
impl State<ServerConnectionData> for ExpectFinished {
        ...
        Self: 'm,
    {
        ...
        let _fin_verified = match ConstantTimeEq::ct_eq(
            &expect_verify_data[..],
            finished.bytes(),
        )
        .into()
