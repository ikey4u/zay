...
use crate::crypto::hpke::{
    EncapsulatedSecret, Hpke, HpkeOpener, HpkePrivateKey, HpkeSuite,
};
...
use crate::msgs::handshake::{
    ClientHelloPayload, EchConfigPayload, EncryptedClientHello,
    EncryptedClientHelloOuter, HandshakeMessagePayload, HandshakePayload,
    HpkeSymmetricCipherSuite, Random, ServerNamePayload,
};
...
pub struct EchServerKey {
    ...
    pub(crate) contents: crate::msgs::handshake::EchConfigContents,
    ...
    pub fn from_raw(
        ...
        if reader.any_left() {
            return Err(Error::General(
                "trailing data after ECH config".into(),
            ));
        }

        let EchConfigPayload::V18(contents) = &config else {
            return Err(Error::General(
                "unsupported ECH config version".into(),
            ));
        };
...
impl EchServerState {
    ...
    pub(crate) fn frontend_info(&self) -> Option<EchFrontendInfo> {
        self.frontend.as_ref().map(|f| EchFrontendInfo {
            ...
            Some(EncryptedClientHello::Outer(_)) => {
                ...
                match EchFrontend::decrypt(&outer, self.keys.index())? {
                    Some((frontend, decrypted)) => {
                        let (inner_input, inner_random) =
                            decrypted.into_message(input);
                    ...
                    None => {
                        self.retry_configs =
                            Some(self.keys.index().retry_configs().to_vec());
                        Ok(None)
                    }
                }
            }
...
impl EchFrontend {
    ...
    fn decrypt(
        ...
        match outer.decrypt_ech(index) {
            ...
            EchDecryptResult::Rejected | EchDecryptResult::NotOffered => {
                Ok(None)
            }
    ...
    fn decrypt_hrr(
        ...
        if ech_ext.config_id != self.config_id
            || ech_ext.cipher_suite != self.cipher_suite
        {
        ...
        let encoded_inner =
            self.opener.open(&aad, &ech_ext.payload.0).map_err(|_| {
...
impl<'a> OuterClientHello<'a> {
    fn from_input(input: &'a Message<'a>) -> Result<Self, Error> {
    ...
    fn decrypt_ech(&self, index: &EchKeyIndex) -> EchDecryptResult {
        let ech_ext = match &self.hello.encrypted_client_hello {
            ...
            Some(EncryptedClientHello::Inner) => {
                return EchDecryptResult::NotOffered
            }
        ...
        for entry in candidates {
            let Ok(mut opener) = entry.hpke_suite.setup_opener(
                &enc,
                &entry.hpke_info,
                &entry.private_key,
            ) else {
            ...
            let Ok(encoded_inner) = opener.open(&aad, &ech_ext.payload.0)
            else {
                continue;
            };
            ...
            match self.decode_inner_hello(encoded_inner) {
                Ok(decrypted) => {
                    return EchDecryptResult::Accepted(decrypted, opener)
                }
                Err(e) => return EchDecryptResult::Fatal(e),
            }
        }
    ...
    fn decode_inner_hello(
        &self,
        encoded_inner: Vec<u8>,
    ) -> Result<DecryptedEch, Error> {
        if encoded_inner.is_empty() {
            return Err(PeerMisbehaved::InvalidEchClientHelloInner.into());
        }

        let inner_hello_encoded =
            self.reconstruct_inner_bytes(&encoded_inner)?;

        // Parse the body after the 4-byte handshake header.
        let mut reader = Reader::init(&inner_hello_encoded[4..]);
        let inner_hello =
            ClientHelloPayload::read(&mut reader).map_err(|_| -> Error {
                PeerMisbehaved::InvalidEchClientHelloInner.into()
            })?;
        ...
        match &inner_hello.extensions.supported_versions {
    ...
    fn compute_aad(
        ...
        let fields = RawClientHello::parse(&aad)
            .ok_or(PeerMisbehaved::InvalidEchClientHelloInner)?;
        let ech = fields
            .iter_extensions()
    ...
    fn reconstruct_inner_bytes(
        ...
        let err =
            || -> Error { PeerMisbehaved::InvalidEchClientHelloInner.into() };
        ...
        out.extend_from_slice(
            &(inner.cipher_suites.len() as u16).to_be_bytes(),
        );
        ...
        for ext in inner.iter_extensions() {
            ...
            if ext.ext_type
                == u16::from(ExtensionType::EncryptedClientHelloOuterExtensions)
            {
                if expanded_outer {
                    return Err(err());
                }
        ...
        out[ext_len_pos..ext_len_pos + 2]
            .copy_from_slice(&ext_total_len.to_be_bytes());
    ...
    /// would not catch.
    fn expand_outer_extensions(
        &self,
        ext_data: &[u8],
        out: &mut Vec<u8>,
    ) -> Result<(), Error> {
        let err =
            || -> Error { PeerMisbehaved::InvalidEchOuterExtension.into() };
        ...
        let list_data = list_reader.take(list_len).ok_or_else(err)?;
...
impl DecryptedEch {
    ...
    fn into_message(
        self,
        outer_input: &Message<'_>,
    ) -> (Message<'static>, Random) {
...
impl EchKeyIndex {
    ...
    pub(crate) fn new(keys: Vec<EchServerKey>) -> Self {
        let max_id = keys.iter().map(|k| k.config_id() as usize).max();
        let mut by_config_id: Vec<Vec<EchKeyEntry>> = (0..max_id
            .map_or(0, |m| m + 1))
            .map(|_| Vec::new())
            .collect();
        ...
        for key in keys {
            ...
            for &cipher_suite in
                &key.contents.key_config.symmetric_cipher_suites
            {
...
mod tests {
    ...
    fn make_v18_contents(config_id: u8) -> crate::msgs::EchConfigContents {
        ...
        crate::msgs::EchConfigContents {
            ...
            public_name: DnsName::try_from("example.com").unwrap().to_owned(),
            extensions: Vec::new(),
        }
    }
    ...
    fn from_raw_rejects_trailing_data() {
        ...
        let result = EchServerKey::from_raw(&config_bytes, vec![0u8; 32], &[]);
        assert!(
            matches!(result, Err(Error::General(msg)) if msg.contains("trailing data"))
        );
    }
    ...
    fn make_hello(random: [u8; 32]) -> ClientHelloPayload {
        ...
        use crate::msgs::{
            ClientExtensions, Compression, Random, SessionId,
    ...
    fn inner_hello_strips_padding() {
        ...
        outer.session_id =
            SessionId::read(&mut Reader::new(&sid_bytes)).unwrap();

        let outer_wrapper = make_outer(&outer, &[]);
        let decrypted = outer_wrapper.decode_inner_hello(encoded).unwrap();

        assert_eq!(decrypted.inner_hello.session_id, outer.session_id);
        assert_eq!(decrypted.inner_hello.random.0, [0x42u8; 32]);
        assert!(matches!(
            decrypted.inner_hello.encrypted_client_hello,
            Some(EncryptedClientHello::Inner)
        ));
    }
    ...
    fn inner_hello_rejects_truly_empty() {
        ...
        assert!(make_outer(&outer, &[]).decode_inner_hello(vec![]).is_err());
    }

    #[test]
    fn inner_hello_rejects_garbage() {
        let outer = make_hello([0x11u8; 32]);
        assert!(make_outer(&outer, &[])
    ...
    fn inner_hello_rejects_missing_marker() {
        ...
        let outer = make_hello([0x11u8; 32]);
        assert!(make_outer(&outer, &[]).decode_inner_hello(encoded).is_err());
    }
    ...
    fn make_inner_encoded(extra_extensions: &[u8]) -> Vec<u8> {
        ...
        if !extra_extensions.is_empty() {
            ...
            let old_ext_len = u16::from_be_bytes([
                encoded[ext_len_pos],
                encoded[ext_len_pos + 1],
            ]) as usize;
    ...
    fn reconstruct_preserves_non_referenced() {
        ...
        let result =
            try_decode_with_extensions(&inner_exts, &raw_ext(0xaaaa, &[0xAA]));
        assert!(result.is_ok());
    }
    ...
    fn reconstruct_rejects_out_of_order() {
        ...
        assert!(try_decode_with_extensions(
            &outer_ext_ref(&[0xaaaa, 0xbbbb]),
            &outer_exts,
        )
        .is_err());
    }
    ...
    fn aad_zeros_ech_payload() {
        ...
        outer_hello.encrypted_client_hello =
            Some(EncryptedClientHello::Outer(ech_outer.clone()));

        let hmp = HandshakeMessagePayload(HandshakePayload::ClientHello(
            outer_hello.clone(),
        ));
        ...
        assert!(!aad.is_empty());
        let has_aa_run = aad.windows(32).any(|w| w.iter().all(|&b| b == 0xAA));
        assert!(!has_aa_run, "AAD should have zeroed ECH payload");

        let has_zero_run =
            aad.windows(32).any(|w| w.iter().all(|&b| b == 0x00));
