...
use crate::msgs::handshake::{
    CertificateChain, CertificatePayloadTls13, HandshakeMessagePayload,
    HandshakePayload, NewSessionTicketPayloadTls13, CERTIFICATE_MAX_SIZE_LIMIT,
};
...
mod client_hello {
    ...
    use crate::msgs::handshake::{
        CertificatePayloadTls13, CertificateRequestExtensions,
        CertificateRequestPayloadTls13, ClientHelloPayload, HelloRetryRequest,
        HelloRetryRequestExtensions, KeyShareEntry, Random,
        ServerEncryptedClientHello, ServerExtensions, ServerExtensionsInput,
    ...
    pub(in crate::server) struct CompleteClientHelloHandling {
        ...
        pub(in crate::server) ech: crate::server::ech::EchServerState,
    }
    ...
    impl CompleteClientHelloHandling {
        fn check_binder(
            ...
            let binder_plaintext = match &client_hello.payload {
                MessagePayload::Handshake { parsed, encoded } => &encoded
                    .bytes()
                    [..encoded.bytes().len() - parsed.total_binder_length()],
                _ => unreachable!(),
            };
        ...
        fn attempt_tls13_ticket_decryption(
            ...
            if self.config.ticketer.enabled() {
                self.config.ticketer.decrypt(ticket).and_then(|plain| {
                    persist::ServerSessionValue::read_bytes(&plain).ok()
                })
            } else {
                self.config.session_storage.take(ticket).and_then(|plain| {
                    persist::ServerSessionValue::read_bytes(&plain).ok()
                })
            }
        }
        ...
        pub(in crate::server) fn handle_client_hello(
            ...
            let shares_ext =
                client_hello.key_shares.as_ref().ok_or_else(|| {
            ...
            let early_data_requested =
                client_hello.early_data_request.is_some();
            ...
            let chosen_share_and_kxg = shares_ext.iter().find_map(|share| {
                (share.group == selected_kxg.name())
                    .then_some((share, selected_kxg))
            });
            ...
            if let Some(psk_offer) = &client_hello.preshared_key_offer {
                ...
                for (i, psk_id) in psk_offer.identities.iter().enumerate() {
                    let maybe_resume_data = self
                        ...
                        .map(|resumedata| {
                            resumedata.set_freshness(
                                psk_id.obfuscated_ticket_age,
                                now,
                            )
                        })
                        .filter(|resumedata| {
            ...
            if let Some(resume) = &resumedata {
                cx.data.received_resumption_data =
                    Some(resume.application_data.0.clone());
                cx.common
                    .peer_certificates
            ...
            let doing_client_auth = if full_handshake {
                let client_auth =
                    emit_certificate_req_tls13(&mut flight, &self.config)?;
    ...
    fn emit_server_hello(
        ...
        let extensions = Box::new(ServerExtensions {
            ...
            custom_order: config.reality_server_extension_order.as_ref().map(
                |order| {
        ...
        if let Some(backend) = ech_backend {
            let confirmation =
                backend.server_hello_confirmation(suite, transcript, &sh);
            sh.payload.patch_encoded(
                crate::server::ech::ECH_CONFIRMATION_SPAN,
                &confirmation,
            );
        }
        ...
        // Do key exchange
        let key_schedule =
            key_schedule_pre_handshake.into_handshake(ckx.secret);
    ...
    fn emit_hello_retry_request(
        ...
        if let Some(backend) = ech.backend() {
            ...
            let confirmation =
                backend.hrr_confirmation(suite, transcript, &placeholder);
            req.extensions.encrypted_client_hello =
                Some(Payload::Owned(confirmation.to_vec()));
        } else if ech.ech_offered() {
            let mut random = vec![0; 8];
            secure_random
                .fill(&mut random)
                .expect("secure random generation failed");
            req.extensions.encrypted_client_hello =
                Some(Payload::Owned(random));
        }
    ...
    fn decide_if_early_data_allowed(
        ...
        let early_data_possible = early_data_requested
            ...
            && resume.alpn.as_ref().map(|p| &p.0[..])
                == cx.common.alpn_protocol.as_deref();

        if early_data_configured
            && early_data_possible
            && !cx.data.early_data.was_rejected()
        {
            EarlyDataDecision::Accepted
        } else {
    ...
    fn emit_encrypted_extensions(
        ...
        if let Some(retry_configs) = ech.retry_configs() {
            ep.extensions.encrypted_client_hello_ack =
                Some(ServerEncryptedClientHello {
                    retry_configs: retry_configs.to_vec(),
                });
        }

        let ee = HandshakeMessagePayload(
            HandshakePayload::EncryptedExtensions(ep.extensions),
        );
    ...
    fn emit_certificate_req_tls13(
        ...
        let cr = CertificateRequestPayloadTls13 {
            ...
            extensions: CertificateRequestExtensions {
                ...
                certificate_compression_algorithms: match config
                    .cert_decompressors
                    .as_slice()
                {
                    &[] => None,
    ...
    fn emit_certificate_verify_tls13(
        ...
        let message =
            construct_server_verify_message(&flight.transcript.current_hash());
        ...
        let cv =
            HandshakeMessagePayload(HandshakePayload::CertificateVerify(cv));
    ...
    fn emit_finished_tls13(
        ...
        let fin = HandshakeMessagePayload(HandshakePayload::Finished(
            verify_data_payload,
        ));
        ...
        flight.finish_with_record_lens(
            cx.common,
            config.reality_handshake_record_lens.as_deref(),
        );

        // Now move to application data keys.  Read key change is deferred until
        // the Finish message is received & validated.
        let traffic = key_schedule.into_traffic_with_client_finished_pending(
            hash_at_server_fin,
            &*config.key_log,
            &randoms.client,
            cx.common,
        );
        if let Some(ticket_record_len) = config
            .reality_handshake_record_lens
            .as_ref()
            .and_then(|lengths| lengths.get(4))
            .copied()
        {
            cx.common
                .send_empty_appdata_with_target_len(ticket_record_len);
        }
        traffic
    }
}
...
impl State<ServerConnectionData> for ExpectCertificate {
        ...
        Self: 'm,
    {
        ...
        let Some((end_entity, intermediates)) = client_cert.split_first()
        else {
            if !mandatory {
...
impl ExpectFinished {
    fn emit_ticket(
        ...
        let plain = get_server_session_value(
            suite, resumption, cx, &nonce, now, age_add,
        )
        .get_encoding();
        ...
        if config.max_early_data_size > 0 {
            if !stateless {
                payload.extensions.max_early_data_size =
                    Some(config.max_early_data_size);
            } else {
...
impl State<ServerConnectionData> for ExpectFinished {
        ...
        Self: 'm,
    {
        ...
        let (key_schedule_traffic, resumption) = key_schedule_before_finished
            .into_traffic(self.transcript.current_hash());
