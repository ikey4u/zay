...
use crate::enums::{
    AlertDescription, CertificateType, CipherSuite, HandshakeType,
...
use crate::msgs::handshake::{
    ClientHelloPayload, HandshakePayload, KeyExchangeAlgorithm, ProtocolName,
    Random, ServerExtensions, ServerExtensionsInput, ServerNamePayload,
...
pub(super) type ServerContext<'a> =
    crate::common_state::Context<'a, ServerConnectionData>;
...
impl ExtensionProcessing {
    ...
    pub(super) fn process_common(
        ...
        if cx.common.is_quic() {
            ...
            match transport_params {
                Some(params) => {
                    cx.common.quic.params = Some(params.to_owned().into_vec())
                }
                None => {
    ...
    pub(super) fn process_tls12(
        ...
        if secure_reneg_offered {
            self.extensions.renegotiation_info =
                Some(PayloadU8::new(Vec::new()));
        }
    ...
    fn process_cert_type_extension(
        ...
        let raw_key_negotation_result = match (
            ...
            (true, true, _) => {
                Ok((extension_type, CertificateType::RawPublicKey))
            }
...
impl ExpectClientHello {
    ...
    pub(super) fn with_certified_key(
        ...
        } else if u16::from(client_hello.client_version)
            < u16::from(ProtocolVersion::TLSv1_2)
        {
            return Err(cx.common.send_fatal_alert(
                AlertDescription::ProtocolVersion,
        ...
        sig_schemes.retain(|scheme| {
            suites::compatible_sigscheme_for_suites(*scheme, &client_suites)
        });
        ...
        let certkey = {
            ...
            match certkey {
                ...
                None if self.ech.is_accepted() => {
                    // The resolver cannot serve the private inner name. Reject
                    // ECH and restart certificate selection from the public
                    // outer ClientHello, matching the ECH server rewind
                    // semantics from upstream rustls.
                    self.ech.rewind();
                    cx.data.ech_accepted = None;
                    cx.data.sni = None;
                    let (outer_hello, outer_sig_schemes) =
                        process_client_hello(outer_m, false, cx)?;
                    return self.with_certified_key(
                        outer_sig_schemes,
                        outer_hello,
                        outer_m,
                        outer_m,
                ...
                None => {
                    return Err(cx.common.send_fatal_alert(
                        ...
                        Error::General(
                            "no server certificate chain resolved".to_owned(),
                        ),
                    ));
                }
            }
        ...
        let transcript = match self.transcript {
            HandshakeHashOrBuffer::Buffer(inner) => {
                inner.start_hash(starting_hash)
            }
        ...
        // Save their Random.
        let server_random = match self.config.reality_server_random {
            Some(random) => Random::from(random),
            None => Random::new(self.config.provider.secure_random)?,
        };
        let randoms =
            ConnectionRandoms::new(client_hello.random, server_random);
        match suite {
    ...
    fn choose_suite_and_kx_group(
        ...
        (SupportedCipherSuite, &'static dyn SupportedKxGroup),
        PeerIncompatible,
    > {
        ...
        for offered_group in client_groups {
            let supported =
                self.config.provider.kx_groups.iter().find(|skxg| {
                    skxg.usable_for_version(selected_version)
                        && skxg.name() == *offered_group
                        && self
                            .config
                            .reality_key_exchange_group
                            .is_none_or(|forced| forced == skxg.name())
                });
        ...
        let first_supported_dhe_kxg =
            if selected_version == ProtocolVersion::TLSv1_2 {
                ...
                ffdhe_possible |=
                    !ffdhe_offered && first_supported_dhe_kxg.is_some();
        ...
        let mut suitable_suites_iter = self
            ...
            .filter(|suite| {
                ...
                && self
                    .config
                    .reality_cipher_suite
                    .is_none_or(|forced| forced == suite.suite())
        ...
        match maybe_skxg {
            ...
            None if suite
                .usable_for_kx_algorithm(KeyExchangeAlgorithm::DHE) =>
            {
                ...
                if let Some(server_selected_ffdhe_skxg) =
                    first_supported_dhe_kxg
                {
...
impl State<ServerConnectionData> for ExpectClientHello {
    ...
    {
        let mut state = *self;
        let inner = state.ech.resolve(&m, state.done_retry)?;
        let selected = inner.as_ref().unwrap_or(&m);
        let (client_hello, sig_schemes) =
            process_client_hello(selected, state.done_retry, cx)?;
        cx.data.ech_accepted = state.ech.frontend_info();
        state.with_certified_key(sig_schemes, client_hello, selected, &m, cx)
    }
...
pub(super) fn process_client_hello<'m>(
    ...
    let sni = match &client_hello.server_name {
        Some(ServerNamePayload::SingleDnsName(dns_name)) => {
            Some(dns_name.to_lowercase_owned())
        }
    ...
    let sig_schemes =
        client_hello.signature_schemes.as_ref().ok_or_else(|| {
            cx.common.send_fatal_alert(
                AlertDescription::HandshakeFailure,
