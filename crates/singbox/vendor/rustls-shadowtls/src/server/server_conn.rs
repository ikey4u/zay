...
use crate::conn::{
    ConnectionCommon, ConnectionCore, UnbufferedConnectionCommon,
};
#[cfg(doc)]
use crate::crypto;
use crate::crypto::CryptoProvider;
use crate::enums::{
    CertificateType, CipherSuite, ProtocolVersion, SignatureScheme,
};
...
use crate::msgs::handshake::{
    ClientHelloPayload, ProtocolName, ServerExtensionsInput,
};
...
use crate::{
    compress, sign, verify, versions, DistinguishedName, KeyLog, NamedGroup,
    WantsVersions,
};
...
pub struct ServerConfig {
    ...
    /// Overrides the TLS 1.3 ServerHello random for protocol compatibility
    /// layers which have already obtained an authenticated random from an
    /// upstream handshake. Ordinary TLS users must leave this as `None`.
    ///
    /// This is a narrow compatibility hook for sing-box REALITY and is not
    /// part of upstream rustls.
    pub reality_server_random: Option<[u8; 32]>,

    /// Forces the TLS 1.3 cipher suite selected for a REALITY cover mirror.
    /// Ordinary TLS users must leave this as `None`.
    pub reality_cipher_suite: Option<CipherSuite>,

    /// Forces the TLS 1.3 key-exchange group selected for a REALITY cover
    /// mirror. Ordinary TLS users must leave this as `None`.
    pub reality_key_exchange_group: Option<NamedGroup>,

    /// Forces the wire order of TLS 1.3 ServerHello extensions for a REALITY
    /// cover mirror. Values are TLS extension type numbers. Ordinary TLS
    /// users must leave this as `None`.
    pub reality_server_extension_order: Option<Vec<u16>>,

    /// Forces the total wire lengths (five-byte header plus ciphertext) of
    /// encrypted TLS 1.3 handshake records for a REALITY cover mirror.
    /// One element denotes the common coalesced flight; four elements denote
    /// EncryptedExtensions, Certificate, CertificateVerify, and Finished.
    /// Ordinary TLS users must leave this as `None`.
    pub reality_handshake_record_lens: Option<Vec<usize>>,
    ...
    /// ECH server keys used to decrypt ClientHelloInner. The default resolver
    /// is empty; applications may replace it to enable server-side ECH.
...
mod connection {
    ...
    use super::{
        Accepted, Accepting, EarlyDataState, ServerConfig,
        ServerConnectionData, ServerExtensionsInput,
    };
    ...
    impl io::Read for ReadEarlyData<'_> {
        ...
        fn read_buf(
            &mut self,
            cursor: core::io::BorrowedCursor<'_>,
        ) -> io::Result<()> {
    ...
    impl ServerConnection {
        ...
        /// Returns information about an ECH offer accepted by this server.
        pub fn ech_accepted(
            &self,
        ) -> Option<&crate::server::ech::EchFrontendInfo> {
            self.inner.core.data.ech_accepted.as_ref()
        }
        ...
        pub fn dangerous_extract_secrets(
            self,
        ) -> Result<ExtractedSecrets, Error> {
            self.inner.dangerous_extract_secrets()
        }

        /// Convert this completed buffered connection into the minimal state
        /// used by an external record layer such as Linux kTLS.
        pub fn dangerous_into_kernel_connection(
            self,
        ) -> Result<
            (ExtractedSecrets, KernelConnection<ServerConnectionData>),
            Error,
        > {
            self.inner.dangerous_into_kernel_connection()
        }
    }
    ...
    impl Acceptor {
        ...
        pub fn accept(
            ...
            let sig_schemes =
                match hs::process_client_hello(&message, false, &mut cx) {
...
#[cfg(feature = "std")]
pub use connection::{
    AcceptedAlert, Acceptor, ReadEarlyData, ServerConnection,
};
...
impl UnbufferedServerConnection {
    ...
    pub fn new(config: Arc<ServerConfig>) -> Result<Self, Error> {
        Ok(Self {
            inner: UnbufferedConnectionCommon::from(
...
impl Accepted {
    ...
    pub fn into_connection(
        ...
        self.connection.enable_secret_extraction =
            config.enable_secret_extraction;

        let state = hs::ExpectClientHello::new(
            config,
            ServerExtensionsInput::default(),
        );
        ...
        let new = match state.with_certified_key(
            ...
            Ok(new) => new,
            Err(err) => {
                return Err((err, AcceptedAlert::from(self.connection)))
            }
        };
    ...
    fn client_hello_payload<'a>(
        message: &'a Message<'_>,
    ) -> &'a ClientHelloPayload {
...
impl EarlyDataState {
    ...
    pub(super) fn take_received_plaintext(
        &mut self,
        bytes: Payload<'_>,
    ) -> bool {
...
pub struct ServerConnectionData {
    ...
    pub(crate) ech_accepted: Option<super::ech::EchFrontendInfo>,
}
