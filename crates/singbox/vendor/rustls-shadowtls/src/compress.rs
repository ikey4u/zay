...
use crate::msgs::handshake::{
    CertificatePayloadTls13, CompressedCertificatePayload,
};
use crate::sync::Arc;
...
mod feat_zlib_rs {
    use zlib_rs::{
        compress_bound, compress_slice, decompress_slice, DeflateConfig,
        InflateConfig, ReturnCode,
    };
    ...
    impl CertDecompressor for ZlibRsDecompressor {
        fn decompress(
            ...
            match decompress_slice(output, input, InflateConfig::default()) {
                (output_filled, ReturnCode::Ok)
                    if output_filled.len() == output_len =>
                {
                    Ok(())
                }
    ...
    impl CertCompressor for ZlibRsCompressor {
        fn compress(
            ...
            let (output_filled, rc) =
                compress_slice(&mut output, &input, config);
            if rc != ReturnCode::Ok {
...
impl CompressionCache {
    ...
    fn compression_for_impl(
        ...
        let (max_size, entries) = match self {
            Self::Enabled(CompressionCacheInner { size, entries }) => {
                (*size, entries)
            }
            _ => unreachable!(),
        };
        ...
        let compressed = compressor
            .compress(encoding.clone(), CompressionLevel::Amortized)?;
    ...
    fn uncached_compression(
        ...
        let uncompressed_len = encoding.len() as u32;
        let compressed =
            compressor.compress(encoding, CompressionLevel::Interactive)?;
...
impl CompressionCacheEntry {
    pub(crate) fn compressed_cert_payload(
        &self,
    ) -> CompressedCertificatePayload<'_> {
...
mod tests {
    ...
    fn test_trivial_pairwise(
        ...
        for level in
            [CompressionLevel::Interactive, CompressionLevel::Amortized]
        {
    ...
    fn test_cache_evicts_lru() {
        ...
        let cert1 =
            CertificatePayloadTls13::new([&cert].into_iter(), Some(b"1"));
        let cert2 =
            CertificatePayloadTls13::new([&cert].into_iter(), Some(b"2"));
        let cert3 =
            CertificatePayloadTls13::new([&cert].into_iter(), Some(b"3"));
        let cert4 =
            CertificatePayloadTls13::new([&cert].into_iter(), Some(b"4"));
