...
use crate::crypto::cipher::{
    make_tls12_aad, AeadKey, InboundOpaqueMessage, Iv, KeyBlockShape,
    MessageDecrypter, MessageEncrypter, Nonce, Tls12AeadAlgorithm,
...
impl Tls12AeadAlgorithm for GcmAlgorithm {
    fn decrypter(
        ...
        let dec_key = aead::LessSafeKey::new(
            aead::UnboundKey::new(self.0, dec_key.as_ref()).unwrap(),
        );
    ...
    fn encrypter(
        ...
        let enc_key = aead::LessSafeKey::new(
            aead::UnboundKey::new(self.0, enc_key.as_ref()).unwrap(),
        );
...
impl MessageDecrypter for ChaCha20Poly1305MessageDecrypter {
    fn decrypt<'a>(
        ...
        let nonce = aead::Nonce::assume_unique_for_key(
            Nonce::new(&self.dec_offset, seq).0,
        );
        let aad = aead::Aad::from(make_tls12_aad(
            seq,
            msg.typ,
...
impl MessageEncrypter for ChaCha20Poly1305MessageEncrypter {
    fn encrypt(
        ...
        let nonce = aead::Nonce::assume_unique_for_key(
            Nonce::new(&self.enc_offset, seq).0,
        );
        let aad = aead::Aad::from(make_tls12_aad(
            seq,
            msg.typ,
