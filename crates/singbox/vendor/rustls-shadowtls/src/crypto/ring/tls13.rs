...
use crate::crypto::cipher::{
    make_tls13_aad, AeadKey, InboundOpaqueMessage, Iv, MessageDecrypter,
...
use crate::msgs::message::{
    InboundPlainMessage, OutboundOpaqueMessage, OutboundPlainMessage,
    PrefixedPayload,
};
use crate::suites::{
    CipherSuiteCommon, ConnectionTrafficSecrets, SupportedCipherSuite,
};
...
pub(crate) static TLS13_CHACHA20_POLY1305_SHA256_INTERNAL: &Tls13CipherSuite =
    &Tls13CipherSuite {
        ...
        aead_alg: &Chacha20Poly1305Aead(AeadAlgorithm(
    ...
    fn encrypter(&self, key: AeadKey, iv: Iv) -> Box<dyn MessageEncrypter> {
        ...
        Box::new(Tls13MessageEncrypter {
            enc_key: aead::LessSafeKey::new(
                aead::UnboundKey::new(self.0, key.as_ref()).unwrap(),
            ),
            iv,
        })
    }
...
impl MessageEncrypter for Tls13MessageEncrypter {
    fn encrypt(
        ...
        self.encrypt_with_padding(msg, seq, 0)
    }

    fn encrypt_with_padding(
        &mut self,
        msg: OutboundPlainMessage<'_>,
        seq: u64,
        padding_len: usize,
        ...
        let total_len =
            self.encrypted_payload_len(msg.payload.len()) + padding_len;
        let mut payload = PrefixedPayload::with_capacity(total_len);

        let nonce =
            aead::Nonce::assume_unique_for_key(Nonce::new(&self.iv, seq).0);
        let aad = aead::Aad::from(make_tls13_aad(total_len));
        payload.extend_from_chunks(&msg.payload);
        payload.extend_from_slice(&msg.typ.to_array());
        payload.extend(core::iter::repeat_n(&0, padding_len));
...
impl Hkdf for RingHkdf {
    ...
    fn hmac_sign(&self, key: &OkmBlock, message: &[u8]) -> crypto::hmac::Tag {
        crypto::hmac::Tag::new(
            hmac::sign(&hmac::Key::new(self.1, key.as_ref()), message).as_ref(),
        )
    }
