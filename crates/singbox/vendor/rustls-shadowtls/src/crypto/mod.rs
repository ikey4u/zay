...
pub use crate::webpki::{
    ...
    verify_tls13_signature_with_raw_key, WebPkiSupportedAlgorithms,
};
#[cfg(all(doc, feature = "tls12"))]
use crate::Tls12CipherSuite;
#[cfg(doc)]
use crate::{
    client, crypto, server, sign, ClientConfig, ConfigBuilder, ServerConfig,
    SupportedCipherSuite, Tls13CipherSuite,
};
use crate::{
    suites, Error, NamedGroup, ProtocolVersion, SupportedProtocolVersion,
};
...
mod static_default {
    ...
    #[cfg(feature = "std")]
    static PROCESS_DEFAULT_PROVIDER: OnceLock<Arc<CryptoProvider>> =
        OnceLock::new();
    #[cfg(not(feature = "std"))]
    static PROCESS_DEFAULT_PROVIDER: OnceBox<Arc<CryptoProvider>> =
        OnceBox::new();
}
