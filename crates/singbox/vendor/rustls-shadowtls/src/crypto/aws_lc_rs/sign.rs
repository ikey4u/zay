...
use pki_types::{
    alg_id, PrivateKeyDer, PrivatePkcs8KeyDer, SubjectPublicKeyInfoDer,
};

use super::ring_like::rand::SystemRandom;
use super::ring_like::signature::{
...
impl EcdsaSigningKey {
    ...
    fn new(
        ...
        let key_pair = match der {
            PrivateKeyDer::Sec1(sec1) => EcdsaKeyPair::from_private_key_der(
                sigalg,
                sec1.secret_sec1_der(),
            )
            .map_err(|_| ())?,
            PrivateKeyDer::Pkcs8(pkcs8) => {
                EcdsaKeyPair::from_pkcs8(sigalg, pkcs8.secret_pkcs8_der())
                    .map_err(|_| ())?
            }
            _ => return Err(()),
        };
...
mod tests {
    ...
    fn cannot_load_invalid_pkcs8_encoding() {
        let key =
            PrivateKeyDer::Pkcs8(PrivatePkcs8KeyDer::from(&b"invalid"[..]));
        assert_eq!(
...
mod benchmarks {
    ...
    const SAMPLE_TLS13_MESSAGE: &[u8] = &[
        ...
        0x79, 0x00, 0x04, 0xca, 0xc4, 0x48, 0x0e, 0x70, 0xf2, 0x1b, 0xa9, 0x1c,
        0x16, 0xca, 0x90, 0x48, 0xbe, 0x28, 0x2f, 0xc7, 0xf8, 0x9b, 0x87, 0x72,
        0x93, 0xda, 0x4d, 0x2f, 0x80, 0x80, 0x60, 0x1a, 0xd3, 0x08, 0xe2, 0xb7,
        0x86, 0x14, 0x1b, 0x54, 0xda, 0x9a, 0xc9, 0x6d, 0xe9, 0x66, 0xb4, 0x9f,
        0xe2, 0x2c,
    ];
}
