...
use aws_lc_rs::digest::{
    SHA256_OUTPUT_LEN, SHA384_OUTPUT_LEN, SHA512_OUTPUT_LEN,
};
...
use crate::crypto::hpke::{
    EncapsulatedSecret, Hpke, HpkeOpener, HpkePrivateKey, HpkePublicKey,
    HpkeSealer, HpkeSuite,
};
use crate::crypto::tls13::{
    expand, HkdfExpander, HkdfPrkExtract, HkdfUsingHmac,
};
...
pub static DH_KEM_P256_HKDF_SHA256_AES_128: &HpkeAwsLcRs<
    AES_128_KEY_LEN,
    SHA256_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_P256_HKDF_SHA256_AES_256: &HpkeAwsLcRs<
    AES_256_KEY_LEN,
    SHA256_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_P384_HKDF_SHA384_AES_128: &HpkeAwsLcRs<
    AES_128_KEY_LEN,
    SHA384_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_P384_HKDF_SHA384_AES_256: &HpkeAwsLcRs<
    AES_256_KEY_LEN,
    SHA384_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_P521_HKDF_SHA512_AES_128: &HpkeAwsLcRs<
    AES_128_KEY_LEN,
    SHA512_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_P521_HKDF_SHA512_AES_256: &HpkeAwsLcRs<
    AES_256_KEY_LEN,
    SHA512_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_X25519_HKDF_SHA256_AES_128: &HpkeAwsLcRs<
    AES_128_KEY_LEN,
    SHA256_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
/// for symmetric encryption.
pub static DH_KEM_X25519_HKDF_SHA256_AES_256: &HpkeAwsLcRs<
    AES_256_KEY_LEN,
    SHA256_OUTPUT_LEN,
> = &HpkeAwsLcRs {
    suite: HpkeSuite {
...
impl<const KEY_SIZE: usize, const KDF_SIZE: usize>
    HpkeAwsLcRs<KEY_SIZE, KDF_SIZE>
{
    ...
    fn key_schedule(
        ...
        let psk_id_hash = labeled_extract_for_prk(
            self.hkdf,
            suite_id,
            None,
            Label::PskIdHash,
        ...
        let info_hash = labeled_extract_for_prk(
            self.hkdf,
            suite_id,
            None,
            Label::InfoHash,
...
impl<const KEY_SIZE: usize, const KDF_SIZE: usize> Hpke
    for HpkeAwsLcRs<KEY_SIZE, KDF_SIZE>
{
    fn seal(
        &self,
        info: &[u8],
...
impl<const KEY_SIZE: usize, const KDF_SIZE: usize> Debug
    for HpkeAwsLcRs<KEY_SIZE, KDF_SIZE>
{
...
impl<const KEY_SIZE: usize, const KDF_SIZE: usize> Sealer<KEY_SIZE, KDF_SIZE> {
    ...
    fn test_only_new(
        ...
        let (shared_secret, enc) =
            suite.dh_kem.test_only_encap(pub_key, sk_e)?;
        let key_schedule = suite.key_schedule(shared_secret, info)?;
        Ok((enc, Self { key_schedule }))
    }
}

impl<const KEY_SIZE: usize, const KDF_SIZE: usize> HpkeSealer
    for Sealer<KEY_SIZE, KDF_SIZE>
{
    fn seal(&mut self, aad: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let key =
            UnboundKey::new(self.key_schedule.aead, &self.key_schedule.key.0)
...
impl<const KEY_SIZE: usize, const KDF_SIZE: usize> Opener<KEY_SIZE, KDF_SIZE> {
    ...
    fn new(
        ...
        Ok(Self {
            key_schedule: suite
                .key_schedule(suite.dh_kem.decap(enc, secret_key)?, info)?,
        })
    }
}

impl<const KEY_SIZE: usize, const KDF_SIZE: usize> HpkeOpener
    for Opener<KEY_SIZE, KDF_SIZE>
{
    fn open(
        &mut self,
        aad: &[u8],
...
struct DhKem<const KDF_SIZE: usize> {
    ...
    key_generator: &'static (dyn Fn() -> Result<(HpkePublicKey, HpkePrivateKey), Error>
                  + Send
                  + Sync),
    hkdf: &'static dyn HkdfPrkExtract,
}
...
impl<const KDF_SIZE: usize> DhKem<KDF_SIZE> {
    ...
    fn encap(
        ...
        let sk_e = agreement::PrivateKey::generate(self.agreement_algorithm)
            .map_err(unspecified_err)?;
    ...
    fn test_only_encap(
        ...
        let sk_e = agreement::PrivateKey::from_private_key(
    ...
    fn encap_impl(
        ...
        let pk_r = agreement::UnparsedPublicKey::new(
            self.agreement_algorithm,
            &recipient.0,
        );
        ...
        let shared_secret = agreement::agree(
            ...
            |dh| Ok(self.extract_and_expand(dh, &kem_context)),
        )
        .map_err(unspecified_err)?;
    ...
    fn decap(
        ...
        let pk_e =
            agreement::UnparsedPublicKey::new(self.agreement_algorithm, &enc.0);
...
fn generate_key_pair(
    ...
    let private_key =
        agreement::PrivateKey::generate(alg).map_err(unspecified_err)?;
...
fn labeled_extract_for_expand(
    ...
    let labeled_ikm =
        [&b"HPKE-v1"[..], &suite_id.encoded(), label.as_ref(), ikm].concat();
...
impl LabeledSuiteId {
    ...
    fn encoded(&self) -> Vec<u8> {
        match self {
            ...
            .concat(),
            Self::Kem(kem) => {
                [&b"KEM"[..], &u16::from(*kem).to_be_bytes()].concat()
            }
        }
    }
}
...
static RING_HKDF_HMAC_SHA256: &HkdfUsingHmac<'static> =
    &HkdfUsingHmac(&HMAC_SHA256);
static RING_HKDF_HMAC_SHA384: &HkdfUsingHmac<'static> =
    &HkdfUsingHmac(&HMAC_SHA384);
static RING_HKDF_HMAC_SHA512: &HkdfUsingHmac<'static> =
    &HkdfUsingHmac(&HMAC_SHA512);
...
mod tests {
    ...
    fn smoke_test() {
        for suite in ALL_SUPPORTED_SUITES {
            ...
            let bad_setup_res =
                suite.setup_sealer(info, &HpkePublicKey(vec![]));
            ...
            let bad_key_res =
                suite.setup_opener(&enc, info, &HpkePrivateKey::from(vec![]));
...
mod rfc_tests {
    ...
    fn check_test_vectors() {
        for (idx, vec) in test_vectors().into_iter().enumerate() {
            ...
            let (enc, mut sealer) =
                hpke.setup_test_sealer(&info, &pk_r, &sk_em).unwrap();
    ...
    impl<const KEY_SIZE: usize, const KDF_SIZE: usize> TestHpke
        for HpkeAwsLcRs<KEY_SIZE, KDF_SIZE>
    {
        fn setup_test_sealer(
            &self,
        ...
        {
            let (encap, sealer) =
                Sealer::test_only_new(self, info, pub_key, sk_em)?;
            Ok((encap, Box::new(sealer)))
        }
    }
    ...
    fn test_vectors() -> Vec<TestVector> {
        serde_json::from_reader(
            &mut File::open(
                "../rustls-provider-test/tests/rfc-9180-test-vectors.json",
            )
