...
use aws_lc_rs::cipher::{
    ...
    UnboundCipherKey, AES_256, AES_256_KEY_LEN, AES_CBC_IV_LEN,
};
...
impl Rfc5077Ticketer {
    fn new() -> Result<Self, Error> {
        ...
        let aes_encrypt_key = UnboundCipherKey::new(&AES_256, &aes_key[..])
        ...
        let aes_decrypt_key = UnboundCipherKey::new(&AES_256, &aes_key[..])
            .map_err(unspecified_err)?;
        let aes_decrypt_key =
        ...
        let hmac_key = hmac::Key::generate(hmac::HMAC_SHA256, &rand)
            .map_err(unspecified_err)?;
...
impl ProducesTickets for Rfc5077Ticketer {
    ...
    fn encrypt(&self, message: &[u8]) -> Option<Vec<u8>> {
        ...
        let dec_ctx =
            self.aes_encrypt_key.encrypt(&mut encrypted_state).ok()?;
        ...
        hmac_data.extend(iv);
        hmac_data
            .extend(u16::try_from(encrypted_state.len()).ok()?.to_be_bytes());
        ...
        let mut ciphertext = Vec::with_capacity(
            self.key_name.len() + iv.len() + encrypted_state.len() + tag.len(),
        );
    ...
    fn decrypt(&self, ciphertext: &[u8]) -> Option<Vec<u8>> {
        ...
        let (alleged_key_name, ciphertext) =
            try_split_at(ciphertext, self.key_name.len())?;
        ...
        let (enc_state, mac) =
            try_split_at(ciphertext, ciphertext.len() - tag_len)?;

        // Reconstitute the HMAC data to verify the tag.
        let mut hmac_data = Vec::with_capacity(
            alleged_key_name.len() + iv.len() + 2 + enc_state.len(),
        );
        ...
        let plaintext =
            self.aes_decrypt_key.decrypt(&mut out, dec_context).ok()?;
...
mod tests {
    ...
    fn ticketrotator_switching_test() {
        let t = Arc::new(
            crate::ticketer::TicketRotator::new(1, make_ticket_generator)
                .unwrap(),
        );
    ...
    fn ticketswitcher_switching_test() {
        #[expect(deprecated)]
        let t = Arc::new(
            crate::ticketer::TicketSwitcher::new(1, make_ticket_generator)
                .unwrap(),
        );
        let now = UnixTime::now();
        let cipher1 = t.encrypt(b"ticket 1").unwrap();
