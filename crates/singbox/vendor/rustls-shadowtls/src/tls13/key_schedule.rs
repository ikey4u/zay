...
use crate::crypto::cipher::{
    AeadKey, Iv, MessageDecrypter, Tls13AeadAlgorithm,
};
use crate::crypto::tls13::{
    expand, Hkdf, HkdfExpander, OkmBlock, OutputLengthError,
};
...
impl KeyScheduleEarly {
    ...
    pub(crate) fn client_early_traffic_secret(
        ...
        match common.side {
            Side::Client => {
                self.ks.set_encrypter(&client_early_traffic_secret, common)
            }
            Side::Server => {
                self.ks.set_decrypter(&client_early_traffic_secret, common)
            }
        }
...
impl KeyScheduleHandshake {
    ...
    pub(crate) fn into_pre_finished_client_traffic(
        ...
        let tag = before_finished.ks.sign_finish(
            &self.client_handshake_traffic_secret,
            &handshake_hash,
        );
...
impl KeyScheduleTrafficWithClientFinishedPending {
    ...
    pub(crate) fn sign_client_finish(
        ...
        self.before_finished.ks.set_decrypter(
            &self.before_finished.current_client_traffic_secret,
            common,
        );
...
impl KeyScheduleTraffic {
    ...
    pub(crate) fn export_keying_material(
        ...
        self.ks.export_keying_material(
    ...
    pub(crate) fn extract_secrets(
        ...
        let client_secrets =
            self.ks.suite.aead_alg.extract_keys(client_key, client_iv)?;
        let server_secrets =
            self.ks.suite.aead_alg.extract_keys(server_key, server_iv)?;
...
impl KeyScheduleSuite {
    ...
    fn sign_verify_data(
        ...
        let hmac_key =
            hkdf_expand_label_block(expander.as_ref(), b"finished", &[]);
    ...
    fn export_keying_material(
        ...
        let h_context =
            self.suite.common.hash_provider.hash(context.unwrap_or(&[]));
...
pub(crate) fn hkdf_expand_label<T: From<[u8; N]>, const N: usize>(
    ...
    hkdf_expand_label_inner(expander, label, context, N, |e, info| {
        expand(e, info)
    })
}
...
fn hkdf_expand_label_slice(
    ...
    hkdf_expand_label_inner(
        ...
        context,
        output.len(),
        |e, info| e.expand_slice(info, output),
    )
}
...
mod tests {
    ...
    use super::{
        derive_traffic_iv, derive_traffic_key, KeySchedule, SecretKind,
    };
    ...
    fn test_vectors() {
        ...
        let hs_start_hash = [
            0xec, 0x14, 0x7a, 0x06, 0xde, 0xa3, 0xc8, 0x84, 0x6c, 0x02, 0xb2,
            0x23, 0x8e, 0x41, 0xbd, 0xdc, 0x9d, 0x89, 0xf9, 0xae, 0xa1, 0x7b,
            0x5e, 0xfd, 0x4d, 0x74, 0x82, 0xaf, 0x75, 0x88, 0x1c, 0x0a,
        ];

        let hs_full_hash = [
            0x75, 0x1a, 0x3d, 0x4a, 0x14, 0xdf, 0xab, 0xeb, 0x68, 0xe9, 0x2c,
            0xa5, 0x91, 0x8e, 0x24, 0x08, 0xb9, 0xbc, 0xb0, 0x74, 0x89, 0x82,
            0xec, 0x9c, 0x32, 0x30, 0xac, 0x30, 0xbb, 0xeb, 0x23, 0xe2,
        ];

        let ecdhe_secret = [
            0xe7, 0xb8, 0xfe, 0xf8, 0x90, 0x3b, 0x52, 0x0c, 0xb9, 0xa1, 0x89,
            0x71, 0xb6, 0x9d, 0xd4, 0x5d, 0xca, 0x53, 0xce, 0x2f, 0x12, 0xbf,
            0x3b, 0xef, 0x93, 0x15, 0xe3, 0x12, 0x71, 0xdf, 0x4b, 0x40,
        ];

        let client_hts = [
            0x61, 0x7b, 0x35, 0x07, 0x6b, 0x9d, 0x0e, 0x08, 0xcf, 0x73, 0x1d,
            0x94, 0xa8, 0x66, 0x14, 0x78, 0x41, 0x09, 0xef, 0x25, 0x55, 0x51,
            0x92, 0x1d, 0xd4, 0x6e, 0x04, 0x01, 0x35, 0xcf, 0x46, 0xab,
        ];

        let client_hts_key = [
            0x62, 0xd0, 0xdd, 0x00, 0xf6, 0x96, 0x19, 0xd3, 0xb8, 0x19, 0x3a,
            0xb4, 0xa0, 0x95, 0x85, 0xa7,
        ];

        let client_hts_iv = [
            0xff, 0xf7, 0x5d, 0xf5, 0xad, 0x35, 0xd5, 0xcb, 0x3c, 0x53, 0xf3,
            0xa9,
        ];

        let server_hts = [
            0xfc, 0xf7, 0xdf, 0xe6, 0x4f, 0xa2, 0xc0, 0x4f, 0x62, 0x35, 0x38,
            0x7f, 0x43, 0x4e, 0x01, 0x42, 0x23, 0x36, 0xd9, 0xc0, 0x39, 0xde,
            0x68, 0x47, 0xa0, 0xb9, 0xdd, 0xcf, 0x29, 0xa8, 0x87, 0x59,
        ];

        let server_hts_key = [
            0x04, 0x67, 0xf3, 0x16, 0xa8, 0x05, 0xb8, 0xc4, 0x97, 0xee, 0x67,
            0x04, 0x7b, 0xbc, 0xbc, 0x54,
        ];

        let server_hts_iv = [
            0xde, 0x83, 0xa7, 0x3e, 0x9d, 0x81, 0x4b, 0x04, 0xc4, 0x8b, 0x78,
            0x09,
        ];

        let client_ats = [
            0xc1, 0x4a, 0x6d, 0x79, 0x76, 0xd8, 0x10, 0x2b, 0x5a, 0x0c, 0x99,
            0x51, 0x49, 0x3f, 0xee, 0x87, 0xdc, 0xaf, 0xf8, 0x2c, 0x24, 0xca,
            0xb2, 0x14, 0xe8, 0xbe, 0x71, 0xa8, 0x20, 0x6d, 0xbd, 0xa5,
        ];

        let client_ats_key = [
            0xcc, 0x9f, 0x5f, 0x98, 0x0b, 0x5f, 0x10, 0x30, 0x6c, 0xba, 0xd7,
            0xbe, 0x98, 0xd7, 0x57, 0x2e,
        ];

        let client_ats_iv = [
            0xb8, 0x09, 0x29, 0xe8, 0xd0, 0x2c, 0x70, 0xf6, 0x11, 0x62, 0xed,
            0x6b,
        ];

        let server_ats = [
            0x2c, 0x90, 0x77, 0x38, 0xd3, 0xf8, 0x37, 0x02, 0xd1, 0xe4, 0x59,
            0x8f, 0x48, 0x48, 0x53, 0x1d, 0x9f, 0x93, 0x65, 0x49, 0x1b, 0x9f,
            0x7f, 0x52, 0xc8, 0x22, 0x29, 0x0d, 0x4c, 0x23, 0x21, 0x92,
        ];

        let server_ats_key = [
            0x0c, 0xb2, 0x95, 0x62, 0xd8, 0xd8, 0x8f, 0x48, 0xb0, 0x2c, 0xbf,
            0xbe, 0xd7, 0xe6, 0x2b, 0xb3,
        ];

        let server_ats_iv = [
            0x0d, 0xb2, 0x8f, 0x98, 0x85, 0x86, 0xa1, 0xb7, 0xe4, 0xd5, 0xc6,
            0x9c,
        ];
    ...
    fn assert_traffic_secret(
        ...
        let traffic_secret =
            ks.derive_logged_secret(kind, hash, &log, &[0; 32]);
        ...
        let expected_key =
            aead::UnboundKey::new(aead_alg, expected_key).unwrap();
...
mod benchmarks {
    ...
    fn bench_sha256(b: &mut test::Bencher) {
        ...
        fn extract_traffic_secret(ks: &KeySchedule, kind: SecretKind) {
            ...
            let hash = [0u8; 32];
            let traffic_secret =
                ks.derive_logged_secret(kind, &hash, &Log, &[0u8; 32]);
            let traffic_secret_expander =
                TLS13_CHACHA20_POLY1305_SHA256_INTERNAL
                    .hkdf_provider
                    .expander_for_okm(&traffic_secret);
