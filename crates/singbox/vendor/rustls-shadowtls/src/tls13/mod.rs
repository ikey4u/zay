...
impl Tls13CipherSuite {
    ...
    pub fn can_resume_from(
        ...
        (prev.common.hash_provider.algorithm()
            == self.common.hash_provider.algorithm())
        .then_some(prev)
    }
...
impl VerifyMessage {
    fn new(
        handshake_hash: &hash::Output,
        context_string_with_0: &[u8; 34],
    ) -> Self {
        let used =
            64 + context_string_with_0.len() + handshake_hash.as_ref().len();
        ...
        hash[..handshake_hash.as_ref().len()]
            .copy_from_slice(handshake_hash.as_ref());
...
const MAX_VERIFY_MSG: usize =
    64 + CLIENT_CONSTANT.len() + hash::Output::MAX_LEN;
