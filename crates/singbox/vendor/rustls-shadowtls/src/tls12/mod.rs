...
use crate::crypto::cipher::{
    AeadKey, MessageDecrypter, MessageEncrypter, Tls12AeadAlgorithm,
};
...
use crate::suites::{
    CipherSuiteCommon, PartiallyExtractedSecrets, SupportedCipherSuite,
};
...
impl ConnectionSecrets {
    ...
    pub(crate) fn make_cipher_pair(&self, side: Side) -> MessageCipherPair {
        ...
        let (client_write_key, key_block) =
            key_block.split_at(shape.enc_key_len);
        let (server_write_key, key_block) =
            key_block.split_at(shape.enc_key_len);
        let (client_write_iv, key_block) =
            key_block.split_at(shape.fixed_iv_len);
    ...
    fn make_key_block(&self) -> Vec<u8> {
        ...
        let len = (shape.enc_key_len + shape.fixed_iv_len) * 2
...
pub(crate) const DOWNGRADE_SENTINEL: [u8; 8] =
