...
use crate::quic::Quic;
use crate::{
    CommonState, ConnectionTrafficSecrets, Error, ProtocolVersion,
...
impl<Data> KernelConnection<Data> {
    ...
    /// Maximum number of records that may be sent with one traffic key before
    /// the negotiated cipher suite's confidentiality bound is exceeded.
    ...
    /// Derive exporter material after the record layer has been handed to an
    /// external implementation such as kTLS.
    pub fn export_keying_material<T: AsMut<[u8]>>(
        ...
        self.state.export_keying_material(
            output.as_mut(),
            label,
            context,
        )?;
...
pub(crate) trait KernelState: Send + Sync {
    fn export_keying_material(
        ...
        Err(Error::General(
            "exporter unavailable for this external TLS state".into(),
        ))
    }
