...
use crate::msgs::handshake::{
    CertificateChain, HandshakeMessagePayload, ProtocolName,
};
use crate::msgs::message::{
    Message, MessagePayload, OutboundChunks, OutboundOpaqueMessage,
...
impl CommonState {
    pub(crate) fn new(side: Side) -> Self {
        Self {
            ...
            received_plaintext: ChunkVecBuffer::new(Some(
                DEFAULT_RECEIVED_PLAINTEXT_LIMIT,
            )),
    ...
    pub(crate) fn process_main_protocol<Data>(
        ...
        match state.handle(&mut cx, msg) {
            ...
            | Err(e @ Error::InappropriateHandshakeMessage { .. }) => {
                Err(self
                    .send_fatal_alert(AlertDescription::UnexpectedMessage, e))
            }
            Err(e) => Err(e),
        }
    }
    ...
    pub(crate) fn send_msg_encrypt_with_target_len(
        &mut self,
        m: PlainMessage,
        target_record_len: Option<usize>,
    ) {
        let iter = self.message_fragmenter.fragment_message(&m);
        let mut first = true;
        for m in iter {
            ...
            first = false;
            self.send_single_fragment_with_padding(m, padding_len);
        }
    }

    pub(crate) fn send_empty_appdata_with_target_len(
        &mut self,
        target_record_len: usize,
    ) {
        ...
        let padding_len = target_record_len
            .checked_sub(5 + self.record_layer.encrypted_len(0))
            .unwrap_or(0);
        self.send_single_fragment_with_padding(message, padding_len);
    }
    ...
    fn send_single_fragment(&mut self, m: OutboundPlainMessage<'_>) {
        self.send_single_fragment_with_padding(m, 0);
    }

    fn send_single_fragment_with_padding(
        &mut self,
        ...
        if m.typ == ContentType::Alert {
            ...
            let em = self
                .record_layer
                .encrypt_outgoing_with_padding(m, padding_len);
                    ...
                    debug_assert!(
                        matches!(
                            ...
                            MessagePayload::Handshake { .. }
                                | MessagePayload::HandshakeFlight(_)
                        ),
    ...
    pub(crate) fn start_encryption_tls12(
        ...
        self.record_layer.prepare_message_encrypter(
            enc,
            secrets.suite().common.confidentiality_limit,
        );
    ...
    pub(crate) fn process_alert(
        ...
        if alert.level == AlertLevel::Warning {
            ...
            {
                return Err(
                    self.send_fatal_alert(AlertDescription::DecodeError, err)
                );
            }
    ...
    fn check_required_size<'a>(
        ...
        if required_size > outgoing_tls.len() {
            return Err(EncryptError::InsufficientSize(
    ...
    pub(crate) fn received_tls13_change_cipher_spec(
        &mut self,
    ) -> Result<(), Error> {
...
impl<'a, const TLS13: bool> HandshakeFlight<'a, TLS13> {
    ...
    pub(crate) fn finish_with_record_lens(
        self,
        common: &mut CommonState,
        target_record_lens: Option<&[usize]>,
    ) {
        let Some(target_record_lens) = target_record_lens else {
            self.finish(common);
            return;
        };
        if target_record_lens.is_empty() {
            self.finish(common);
            return;
        }
        debug_assert!(TLS13);
        if target_record_lens.len() == 1 {
            let message = Message {
                version: ProtocolVersion::TLSv1_3,
                payload: MessagePayload::HandshakeFlight(Payload::new(
                    self.body,
            ...
            common.send_msg_encrypt_with_target_len(
                message.into(),
                target_record_lens.first().copied(),
            );
            return;
        }
        ...
        while offset < self.body.len() {
            debug_assert!(offset + 4 <= self.body.len());
            let body_len = (usize::from(self.body[offset + 1]) << 16)
                | (usize::from(self.body[offset + 2]) << 8)
                | usize::from(self.body[offset + 3]);
            let end = offset + 4 + body_len;
            debug_assert!(end <= self.body.len());
            let target = target_record_lens.get(record_index).copied();
            common.send_msg_encrypt_with_target_len(
                PlainMessage {
                    typ: ContentType::Handshake,
                    version: ProtocolVersion::TLSv1_3,
                    payload: Payload::Owned(self.body[offset..end].to_vec()),
                },
                target,
            );
            offset = end;
