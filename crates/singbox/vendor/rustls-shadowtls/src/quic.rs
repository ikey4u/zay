...
mod connection {
    ...
    impl Connection {
        ...
        pub fn export_keying_material<T: AsMut<[u8]>>(
            ...
            match self {
                Self::Client(conn) => {
                    conn.core.export_keying_material(output, label, context)
                }
                Self::Server(conn) => {
                    conn.core.export_keying_material(output, label, context)
                }
        ...
        pub fn new_with_alpn(
            ...
            let exts = ClientExtensionsInput {
                transport_parameters: Some(match quic_version {
                    Version::V1Draft => {
                        TransportParameters::QuicDraft(Payload::new(params))
                    }
            ...
            let mut inner =
                ConnectionCore::for_client(config, name, exts, Protocol::Quic)?;
    ...
    impl ServerConnection {
        ...
        pub fn ech_accepted(&self) -> Option<&crate::server::EchFrontendInfo> {
    ...
    impl<Data> From<ConnectionCore<Data>> for ConnectionCommon<Data> {
        fn from(core: ConnectionCore<Data>) -> Self {
            Self {
                ...
                sendable_plaintext: ChunkVecBuffer::new(Some(
...
#[cfg(feature = "std")]
pub use connection::{
    ClientConnection, Connection, ConnectionCommon, ServerConnection,
};
...
impl DirectionalKeys {
    pub(crate) fn new(
        ...
        let builder =
            KeyBuilder::new(secret, version, quic, suite.hkdf_provider);
        Self {
...
impl PacketKeySet {
    fn new(secrets: &Secrets) -> Self {
        ...
        let (version, alg, hkdf) =
            (secrets.version, secrets.quic, secrets.suite.hkdf_provider);
        Self {
...
impl<'a> KeyBuilder<'a> {
    ...
    pub(crate) fn packet_key(&self) -> Box<dyn PacketKey> {
        ...
        let packet_iv = hkdf_expand_label(
            self.expander.as_ref(),
            self.version.packet_iv_label(),
            &[],
        );
...
impl Keys {
    ...
    pub fn initial(
        ...
        let secrets = Secrets {
            version,
            client: hkdf_expand_label_block(
                hs_secret.as_ref(),
                CLIENT_LABEL,
                &[],
            ),
            server: hkdf_expand_label_block(
                hs_secret.as_ref(),
                SERVER_LABEL,
                &[],
            ),
    ...
    fn new(secrets: &Secrets) -> Self {
        let (local, remote) = secrets.local_remote();
        Self {
            local: DirectionalKeys::new(
                secrets.suite,
                secrets.quic,
                local,
                secrets.version,
            ),
            remote: DirectionalKeys::new(
                secrets.suite,
                secrets.quic,
                remote,
                secrets.version,
