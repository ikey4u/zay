...
use webpki::{
    CertRevocationList, ExpirationPolicy, RevocationCheckDepth,
    UnknownStatusPolicy,
};
...
use crate::sync::Arc;
use crate::verify::{
    ClientCertVerified, ClientCertVerifier, DigitallySignedStruct,
    HandshakeSignatureValid, NoClientAuth,
};
use crate::webpki::parse_crls;
use crate::webpki::verify::{
    verify_tls12_signature, verify_tls13_signature, ParsedCertificate,
};
#[cfg(doc)]
...
mod tests {
    ...
    fn load_crls(
        ...
        crls_der
            .iter()
            .map(|pem_bytes| {
                CertificateRevocationListDer::from_pem_slice(pem_bytes).unwrap()
            })
            .collect()
    }
    ...
    fn test_crls() -> Vec<CertificateRevocationListDer<'static>> {
        load_crls(&[
            ...
            .as_slice(),
            include_bytes!("../../../test-ca/rsa-2048/client.revoked.crl.pem")
                .as_slice(),
        ])
    }
    ...
    fn test_with_crls_multiple_calls() {
        ...
        let extra_crls = load_crls(&[include_bytes!(
            "../../../test-ca/eddsa/client.revoked.crl.pem"
        )
        .as_slice()]);
        let builder = WebPkiClientVerifier::builder_with_provider(
            test_roots(),
    ...
    fn smoke() {
        let all = vec![
            ...
            VerifierBuilderError::InvalidCrl(
                crate::CertRevocationListError::ParseError,
            ),
        ];
