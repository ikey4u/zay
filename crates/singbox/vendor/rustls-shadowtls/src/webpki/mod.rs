...
use crate::error::{
    CertRevocationListError, CertificateError, Error, ExtendedKeyPurpose,
    OtherError,
};
#[cfg(feature = "std")]
use crate::sync::Arc;
...
impl fmt::Display for VerifierBuilderError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NoRootAnchors => {
                write!(f, "no root trust anchors were provided")
            }
            Self::InvalidCrl(e) => {
                write!(f, "provided CRL could not be parsed: {e:?}")
            }
        }
    }
}
...
fn parse_crls(
    ...
    crls.iter()
        .map(|der| {
            OwnedCertRevocationList::from_der(der.as_ref()).map(Into::into)
        })
