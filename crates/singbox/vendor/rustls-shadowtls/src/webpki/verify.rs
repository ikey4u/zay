...
impl WebPkiSupportedAlgorithms {
    ...
    fn convert_scheme(
        ...
        self.mapping
            .iter()
            .filter_map(
                |item| if item.0 == scheme { Some(item.1) } else { None },
            )
            .next()
            .ok_or_else(|| {
                PeerMisbehaved::SignedHandshakeWithUnadvertisedSigScheme.into()
            })
    }
...
pub fn verify_tls13_signature_with_raw_key(
    ...
    let raw_key =
        webpki::RawPublicKeyEntity::try_from(spki).map_err(pki_error)?;
    let alg = supported_schemes.convert_scheme(dss.scheme)?[0];
