    ...
    fn read(r: &mut Reader<'_>) -> Result<Self, InvalidMessage> {
        match r.take(8) {
            Some(&[a, b, c, d, e, f, g, h]) => {
                Ok(Self::from_be_bytes([a, b, c, d, e, f, g, h]))
            }
...
impl<'a, T: Codec<'a> + TlsListElement + Debug> Iterator
    for TlsListIter<'a, T>
{
...
impl ListLength {
    pub(crate) fn read(
        ...
        Ok(match self {
            Self::NonZeroU8 { empty_error } => {
                match usize::from(u8::read(r)?) {
                    0 => return Err(*empty_error),
                    len => len,
                }
            }
            Self::U16 => usize::from(u16::read(r)?),
            Self::NonZeroU16 { empty_error } => {
                match usize::from(u16::read(r)?) {
                    0 => return Err(*empty_error),
                    len => len,
                }
            }
    ...
    fn drop(&mut self) {
        match self.size_len {
            ...
            ListLength::U16 | ListLength::NonZeroU16 { .. } => {
                ...
                let out: &mut [u8; 2] = (&mut self.buf
                    [self.len_offset..self.len_offset + 2])
                    .try_into()
                    .unwrap();
            ...
            ListLength::U24 { .. } => {
                ...
                let out: &mut [u8; 3] = (&mut self.buf
                    [self.len_offset..self.len_offset + 3])
                    .try_into()
                    .unwrap();
