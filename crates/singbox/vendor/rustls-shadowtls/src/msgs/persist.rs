...
impl ClientSessionCommon {
    ...
    pub(crate) fn compatible_config(
        ...
        let same_creds =
            Weak::ptr_eq(&Arc::downgrade(client_creds), &self.client_creds);
...
impl ServerSessionValue {
    ...
    pub(crate) fn set_freshness(
        ...
        let client_age_ms =
            obfuscated_client_age_ms.wrapping_sub(self.age_obfuscation_offset);
        let server_age_ms =
            (time_now.as_secs().saturating_sub(self.creation_time_sec) as u32)
                .saturating_mul(1000);
...
mod tests {
    ...
    fn serversessionvalue_no_sni() {
        let bytes = [
            0x00, 0x03, 0x03, 0xc0, 0x23, 0x03, 0x01, 0x02, 0x03, 0x00, 0x00,
