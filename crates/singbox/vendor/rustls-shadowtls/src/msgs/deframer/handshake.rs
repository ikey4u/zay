...
impl HandshakeDeframer {
    ...
    pub(crate) fn input_message(
        ...
        if let Some(_last_incomplete) =
            self.spans.last().filter(|span| !span.is_complete())
        {
    ...
    fn coalesce_one(
        ...
        for (i, span) in
            DissectHandshakeIter::new(msg, &delocator.locator()).enumerate()
        {
...
impl Iterator for DissectHandshakeIter<'_, '_> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let (header, rest) =
            mem::take(&mut self.payload).split_at(HANDSHAKE_HEADER_LEN);
...
mod tests {
    ...
    fn handshake_flight() {
        ...
        let mut input =
            include_bytes!("../../testdata/handshake-test.1.bin").to_vec();
