...
use super::enums::{
    ClientCertificateType, Compression, ECCurveType, ExtensionType,
    KeyUpdateRequest, NamedGroup,
};
use super::handshake::{
    CertificateChain, CertificateEntry, CertificateExtensions,
    CertificatePayloadTls13, CertificateRequestExtensions,
    CertificateRequestPayload, CertificateRequestPayloadTls13,
    CertificateStatus, CertificateStatusRequest, ClientExtensions,
    ClientHelloPayload, ClientSessionTicket, CompressedCertificatePayload,
    DistinguishedName, EcParameters, EncryptedClientHello,
    ...
    PresharedKeyIdentity, PresharedKeyOffer, ProtocolName, PskKeyExchangeModes,
    Random, ServerDhParams, ServerEcdhParams, ServerEncryptedClientHello,
    ServerExtensions, ServerHelloPayload, ServerKeyExchange,
    ServerKeyExchangeParams, ServerKeyExchangePayload, ServerNamePayload,
...
fn rejects_duplicate_names_in_sni_extension() {
    assert_eq!(
        ClientExtensions::read_bytes(&[
            0, 14, 0, 0, 0, 10, 0, 8, 0, 0, 1, b'a', 0, 0, 1, b'b',
        ])
        .unwrap_err(),
...
fn can_round_trip_cert_status_req_for_ocsp() {
    ...
    let csr =
        CertificateStatusRequest::read(&mut Reader::init(&bytes)).unwrap();
    println!("{csr:?}");
...
fn client_extensions_decode_checks_duplicates() {
    ...
    assert_eq!(
        ClientExtensions::read_bytes(
            b"\x00\x08\x00\x2a\x00\x00\x00\x2a\x00\x00"
        )
        .unwrap_err(),
    ...
    assert_eq!(
        ClientExtensions::read_bytes(
            b"\x00\x08\xff\xff\x00\x00\xff\xff\x00\x00"
        )
        .unwrap_err(),
...
fn client_extensions_ordering() {
    ...
    fn encoding_with_order(
        order_seed: u16,
        exts: &ClientExtensions<'_>,
    ) -> Vec<u8> {
    ...
    for seed in 0..=0xffff {
        ...
        assert!(encoding_with_order(seed, &psk_and_ech_with_contiguous)
            .ends_with(
                b"\x00\x17\x00\x00\
                  \xfe\x0d\x00\x01\x01\
                  \x00\x29\x00\x04\x00\x00\x00\x00"
        ...
        // just PSK
        assert!(encoding_with_order(seed, &psk)
            .ends_with(b"\x00\x29\x00\x04\x00\x00\x00\x00"));

        // just ECH
        assert!(
            encoding_with_order(seed, &ech).ends_with(b"\xfe\x0d\x00\x01\x01")
        );
    }
}
...
fn can_round_trip_all_tls13_handshake_payloads() {
    for hm in all_tls13_handshake_payloads().iter() {
        ...
        let other = HandshakeMessagePayload::read_version(
            &mut rd,
...
fn cannot_read_message_hash_from_network() {
    let mh = HandshakeMessagePayload(HandshakePayload::MessageHash(
...
fn can_decode_server_hello_from_api_devicecheck_apple_com() {
    let data =
        include_bytes!("../testdata/hello-api.devicecheck.apple.com.bin");
...
fn sample_client_hello_payload() -> ClientHelloPayload {
    ClientHelloPayload {
        ...
        extensions: Box::new(ClientExtensions {
            ...
            key_shares: Some(vec![KeyShareEntry::new(
                NamedGroup::X25519,
                &[1, 2, 3][..],
            )]),
            ...
            extended_master_secret_request: Some(()),
            certificate_status_request: Some(
            ...
            early_data_request: Some(()),
            certificate_compression_algorithms: Some(vec![
...
fn sample_server_hello_payload() -> ServerHelloPayload {
    ServerHelloPayload {
        ...
        extensions: Box::new(ServerExtensions {
            ...
            selected_protocol: Some(SingleProtocolName::new(
                ProtocolName::from(vec![0]),
            )),
            key_share: Some(KeyShareEntry::new(
                NamedGroup::X25519,
                &[1, 2, 3][..],
            )),
...
fn all_tls12_handshake_payloads() -> Vec<HandshakeMessagePayload<'static>> {
    vec![
        ...
        HandshakeMessagePayload(HandshakePayload::ClientHello(
            sample_client_hello_payload(),
        )),
        HandshakeMessagePayload(HandshakePayload::ServerHello(
            sample_server_hello_payload(),
        )),
        HandshakeMessagePayload(HandshakePayload::HelloRetryRequest(
            sample_hello_retry_request(),
        )),
        HandshakeMessagePayload(HandshakePayload::Certificate(
            CertificateChain(vec![CertificateDer::from(vec![1, 2, 3])]),
        )),
        ...
        HandshakeMessagePayload(HandshakePayload::ClientKeyExchange(
        ...
        HandshakeMessagePayload(HandshakePayload::Finished(Payload::Borrowed(
            &[1, 2, 3],
        ))),
