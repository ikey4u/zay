...
pub use outbound::{
    OutboundChunks, OutboundOpaqueMessage, OutboundPlainMessage,
    PrefixedPayload,
};
...
impl<'a> MessagePayload<'a> {
    ...
    pub fn new(
        ...
        match typ {
            ContentType::ApplicationData => {
                Ok(Self::ApplicationData(Payload::Borrowed(payload)))
            }
            ContentType::Alert => {
                AlertMessagePayload::read(&mut r).map(MessagePayload::Alert)
            }
            ContentType::Handshake => HandshakeMessagePayload::read_version(
                &mut r, vers,
            )
            ...
            ContentType::ChangeCipherSpec => {
                ChangeCipherSpecPayload::read(&mut r)
    ...
    pub fn content_type(&self) -> ContentType {
        match self {
            ...
            Self::Handshake { .. } | Self::HandshakeFlight(_) => {
