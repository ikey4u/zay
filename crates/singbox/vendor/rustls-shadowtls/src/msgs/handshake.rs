...
use crate::enums::{
    CertificateCompressionAlgorithm, CertificateType, CipherSuite,
    EchClientHelloType, HandshakeType, ProtocolVersion, SignatureScheme,
};
...
use crate::log::warn;
use crate::msgs::base::{
    MaybeEmpty, NonEmpty, Payload, PayloadU16, PayloadU24, PayloadU8,
};
use crate::msgs::codec::{
    self, Codec, LengthPrefixedBuffer, ListLength, Reader, TlsListElement,
    TlsListIter,
};
use crate::msgs::enums::{
    CertificateStatusType, ClientCertificateType, Compression, ECCurveType,
    ECPointFormat, EchVersion, ExtensionType, HpkeAead, HpkeKdf, HpkeKem,
...
impl ServerNamePayload<'_> {
    fn into_owned(self) -> ServerNamePayload<'static> {
        match self {
            Self::SingleDnsName(d) => {
                ServerNamePayload::SingleDnsName(d.to_owned())
            }
...
impl Codec<'_> for PskKeyExchangeModes {
    fn encode(&self, bytes: &mut Vec<u8>) {
        let inner =
            LengthPrefixedBuffer::new(PskKeyExchangeMode::SIZE_LEN, bytes);
        if self.psk_dhe {
...
impl TlsListElement for CertificateCompressionAlgorithm {
    const SIZE_LEN: ListLength = ListLength::NonZeroU8 {
        empty_error: InvalidMessage::IllegalEmptyList(
...
impl TransportParameters<'_> {
    pub(crate) fn into_owned(self) -> TransportParameters<'static> {
        match self {
            Self::QuicDraft(v) => {
                TransportParameters::QuicDraft(v.into_owned())
            }
...
impl ClientExtensions<'_> {
    pub(crate) fn into_owned(self) -> ClientExtensions<'static> {
        ...
        ClientExtensions {
            ...
            transport_parameters: transport_parameters.map(|x| x.into_owned()),
            renegotiation_info,
            transport_parameters_draft: transport_parameters_draft
                .map(|x| x.into_owned()),
            encrypted_client_hello,
            encrypted_client_hello_outer,
            order_seed,
    ...
    pub(crate) fn has_exact_extension(
        &self,
    ...
    pub(crate) fn orderable_extension_types(&self) -> Vec<ExtensionType> {
        ...
        orderable.retain(|ext| {
            !self.is_final_extension(*ext)
                && !self.contiguous_extensions.contains(ext)
        });
        orderable
    }

    pub(crate) fn set_custom_order(
        &mut self,
    ...
    pub(crate) fn used_extensions_in_encoding_order(
        &self,
    ) -> Vec<ExtensionType> {
    ...
    fn order_insensitive_extensions_in_random_order(
        &self,
    ) -> Vec<ExtensionType> {
        ...
        order.sort_by_cached_key(|new_ext| {
            let seed =
                ((self.order_seed as u32) << 16) | (u16::from(*new_ext) as u32);
    ...
    fn exact_extension_payload(
        &self,
        item: ExtensionType,
    ) -> Option<&Payload<'static>> {
        self.exact_extensions
            .iter()
            .find_map(|(extension_type, payload)| {
                (*extension_type == item).then_some(payload)
            })
    }

    fn raw_extension_payload(
        &self,
        item: ExtensionType,
    ) -> Option<&Payload<'static>> {
        self.raw_extensions
    ...
    fn grease_extension_payload(
        &self,
        item: ExtensionType,
    ) -> Option<&Payload<'static>> {
        self.grease_extensions.iter().find_map(
            |(_, extension_type, payload)| {
                (*extension_type == item).then_some(payload)
            },
        )
    }
...
extension_struct! {
    pub(crate) struct ServerExtensions<'a> {
        ...
        /// Optional exact wire order for emitted ServerHello extensions.
        pub(crate) custom_order: Option<Vec<ExtensionType>>,
    }
}
...
impl ClientHelloPayload {
    pub(crate) fn ech_inner_encoding(
        &self,
        ...
        self.payload_encode(
            &mut bytes,
            Encoding::EchInnerHello { to_compress },
        );
        bytes
    }
    ...
    pub(crate) fn payload_encode(
        ...
        for e in &to_compress {
            ...
            compressed
                .exact_extensions
                .retain(|(extension, _)| extension != e);
            compressed
                .raw_extensions
                .retain(|(extension, _)| extension != e);
        }
        ...
        self.key_shares
            ...
            .map(|entries| {
                has_duplicates::<_, _, u16>(
                    entries.iter().map(|kse| u16::from(kse.group)),
                )
            })
            .unwrap_or_default()
    }

    pub(crate) fn has_certificate_compression_extension_with_duplicates(
        &self,
    ) -> bool {
    ...
    fn payload_encode(&self, bytes: &mut Vec<u8>, purpose: Encoding) {
        ...
        match purpose {
            ...
            Encoding::EchConfirmation
                if self.extensions.encrypted_client_hello.is_some() =>
            {
                let hrr_confirmation = [0u8; 8];
                HelloRetryRequestExtensions {
                    encrypted_client_hello: Some(Payload::Borrowed(
                        &hrr_confirmation,
                    )),
            ...
            entries: certs
                ...
                .map(|(cert, ocsp)| {
                    ...
                    if let Some(ocsp) = ocsp {
                        e.extensions.status =
                            Some(CertificateStatus::new(ocsp));
                    }
                    e
                })
                .collect(),
...
impl ServerDhParams {
    ...
    pub(crate) fn as_ffdhe_group(&self) -> FfdheGroup<'_> {
        FfdheGroup::from_params_trimming_leading_zeros(
            &self.dh_p.0,
            &self.dh_g.0,
        )
    }
    ...
    pub(crate) fn new(kx: &dyn ActiveKeyExchange) -> Self {
        match kx.group().key_exchange_algorithm() {
            ...
            KeyExchangeAlgorithm::ECDHE => {
                Self::Ecdh(ServerEcdhParams::new(kx))
            }
        }
    }
...
impl CompressedCertificatePayload<'_> {
    ...
    pub(crate) fn as_borrowed(&self) -> CompressedCertificatePayload<'_> {
        CompressedCertificatePayload {
            ...
            compressed: PayloadU24(Payload::Borrowed(
                self.compressed.0.bytes(),
            )),
        }
    }
}
...
impl HandshakePayload<'_> {
    ...
    pub(crate) fn handshake_type(&self) -> HandshakeType {
        ...
        match self {
            ...
            CertificateRequest(_) | CertificateRequestTls13(_) => {
            ...
            NewSessionTicket(_) | NewSessionTicketTls13(_) => {
    ...
    fn into_owned(self) -> HandshakePayload<'static> {
        ...
        match self {
            ...
            EncryptedExtensions(x) => {
                EncryptedExtensions(Box::new(x.into_owned()))
            }
...
impl<'a> HandshakeMessagePayload<'a> {
    pub(crate) fn read_version(
        ...
        let payload = match typ {
            ...
            HandshakeType::ClientHello => HandshakePayload::ClientHello(
                ClientHelloPayload::read(&mut sub)?,
            ),
            ...
            HandshakeType::CertificateVerify => {
                HandshakePayload::CertificateVerify(
                    DigitallySignedStruct::read(&mut sub)?,
                )
            }
            HandshakeType::NewSessionTicket
                if vers == ProtocolVersion::TLSv1_3 =>
            ...
            HandshakeType::EncryptedExtensions => {
                HandshakePayload::EncryptedExtensions(Box::new(
                    ServerExtensions::read(&mut sub)?,
                ))
            }
            ...
            HandshakeType::CertificateStatus => {
                HandshakePayload::CertificateStatus(CertificateStatus::read(
                    &mut sub,
                )?)
            }
...
impl EchConfigContents {
    ...
    pub(crate) fn has_duplicate_extension(&self) -> bool {
        has_duplicates::<_, _, u16>(
            self.extensions.iter().map(|ext| ext.ext_type()),
        )
    }
...
impl Codec<'_> for EchConfigContents {
    fn encode(&self, bytes: &mut Vec<u8>) {
        ...
        PayloadU8::<MaybeEmpty>::encode_slice(
            dns_name.as_ref().as_ref(),
            bytes,
        );
        self.extensions.encode(bytes);
    }
    ...
    fn read(r: &mut Reader<'_>) -> Result<Self, InvalidMessage> {
        Ok(Self {
            ...
            public_name: {
                DnsName::try_from(
                    PayloadU8::<MaybeEmpty>::read(r)?.0.as_slice(),
                )
...
impl Codec<'_> for EchConfigPayload {
    ...
    fn read(r: &mut Reader<'_>) -> Result<Self, InvalidMessage> {
        ...
        Ok(match version {
            EchVersion::V18 => {
                Self::V18(EchConfigContents::read(&mut contents)?)
            }
            _ => {
...
fn has_duplicates<I: IntoIterator<Item = E>, E: Into<T>, T: Eq + Ord>(
    iter: I,
) -> bool {
...
mod tests {
    ...
    fn test_ech_config_mandatory_exts() {
        let mandatory_unknown_ext =
            EchConfigExtension::Unknown(UnknownExtension {
