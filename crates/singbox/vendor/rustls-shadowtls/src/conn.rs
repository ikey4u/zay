...
use crate::common_state::{
    CommonState, Context, IoState, State, DEFAULT_BUFFER_LIMIT,
};
use crate::enums::{AlertDescription, ContentType, ProtocolVersion};
use crate::error::{Error, PeerMisbehaved};
use crate::log::trace;
use crate::msgs::deframer::buffers::{
    BufferProgress, DeframerVecBuffer, Delocator, Locator,
};
    ...
    impl<T> PlaintextSink for ConnectionCommon<T> {
        ...
        fn write_vectored(
            ...
            let payload = match bufs.len() {
                ...
                _ => {
                    payload_owner =
                        bufs.iter().map(|io_slice| io_slice.deref()).collect();
...
impl<Data> ConnectionCommon<Data> {
    ...
    /// Extract secrets and retain the TLS 1.3 post-handshake state required by
    /// an external record layer such as Linux kTLS.
    ///
    /// This is the buffered-connection counterpart of the unbuffered API. It
    /// is safe after an I/O driver has completed and flushed the handshake;
    /// the core conversion still rejects pending TLS records.
    pub fn dangerous_into_kernel_connection(
        self,
    ) -> Result<(ExtractedSecrets, KernelConnection<Data>), Error> {
        self.core.dangerous_into_kernel_connection()
    }
    ...
    /// Remove bytes which have already been read from the transport while
    /// preserving their wire order for a protocol that is about to take over
    /// the underlying stream.
    ///
    /// The first vector contains decrypted application data that has not yet
    /// been consumed through [`ConnectionCommon::reader`].  The second vector
    /// contains raw transport bytes that rustls read ahead but did not consume
    /// as complete TLS records.  This is intentionally a narrow escape hatch
    /// for protocols such as VLESS Vision, whose negotiated direct mode ends
    /// the outer TLS record layer without closing the underlying connection.
    /// Ordinary TLS users must not call this method.
    #[cfg(feature = "std")]
    pub fn dangerous_take_read_ahead(&mut self) -> (Vec<u8>, Vec<u8>) {
        let plaintext_len = self.core.common_state.received_plaintext.len();
        ...
        let raw = self.deframer_buffer.filled().to_vec();
        self.deframer_buffer.discard(raw.len());
        (plaintext, raw)
    }
...
impl<Data> ConnectionCommon<Data> {
        ...
        T: io::Read + io::Write,
    {
        ...
        loop {
            ...
            match (eof, until_handshaked, self.is_handshaking(), blocked) {
                ...
                (_, _, _, Some((e, _))) if rdlen == 0 && wrlen == 0 => {
                    return Err(e)
                }
    ...
    pub(crate) fn first_handshake_message(
        ...
        let res = self
            ...
            .map(|opt| {
                opt.map(|pm| Message::try_from(pm).map(|m| m.into_owned()))
            });
        ...
        match res? {
            ...
            Some(Err(err)) => {
                Err(self.send_fatal_alert(AlertDescription::DecodeError, err))
            }
            None => Ok(None),
        }
    }
...
impl<Data> ConnectionCore<Data> {
    ...
    fn process_more_input<'b>(
        ...
        loop {
            let mut iter =
                DeframerIter::new(&mut buffer[buffer_progress.processed()..]);
            ...
            if !self.hs_deframer.is_aligned()
            ...
            let unborrowed =
                InboundUnborrowedMessage::unborrow(&locator, message);
            ...
            let message = unborrowed.reborrow(&Delocator::new(buffer));
            self.hs_deframer.input_message(
                message,
                &locator,
    ...
    pub(crate) fn dangerous_extract_secrets(
        self,
    ) -> Result<ExtractedSecrets, Error> {
...
impl InboundUnborrowedMessage {
    ...
    fn reborrow<'b>(
        self,
        delocator: &Delocator<'b>,
    ) -> InboundPlainMessage<'b> {
        InboundPlainMessage {
            typ: self.typ,
