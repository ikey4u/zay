...
use crate::crypto::hpke::{
    EncapsulatedSecret, Hpke, HpkePublicKey, HpkeSealer, HpkeSuite,
};
...
use crate::msgs::handshake::{
    ClientExtensions, ClientHelloPayload, EchConfigContents, EchConfigPayload,
    Encoding, EncryptedClientHello, EncryptedClientHelloOuter,
    HandshakeMessagePayload, HandshakePayload, HelloRetryRequest,
    HpkeKeyConfig, HpkeSymmetricCipherSuite, PresharedKeyBinder,
...
use crate::tls13::key_schedule::{
    server_ech_hrr_confirmation_secret, KeyScheduleEarly,
    KeyScheduleHandshakeStart,
};
use crate::CipherSuite;
use crate::{
    AlertDescription, ClientConfig, CommonState, EncryptedClientHelloError,
    Error, PeerIncompatible, PeerMisbehaved, ProtocolVersion, Tls13CipherSuite,
};
...
impl EchConfig {
    ...
    pub fn new(
        ...
        let ech_configs =
            Vec::<EchConfigPayload>::read(&mut Reader::init(&ech_config_list))
                .map_err(|_| {
                    Error::InvalidEncryptedClientHello(
        ...
        for (_i, config) in ech_configs.iter().enumerate() {
            ...
            if contents.has_unknown_mandatory_extension()
                || contents.has_duplicate_extension()
            {
            ...
            for cipher_suite in &key_config.symmetric_cipher_suites {
                ...
                if let Some(hpke) =
                    hpke_suites.iter().find(|hpke| hpke.suite() == suite)
                {
                    debug!(
...
impl EchGreaseConfig {
    ...
    pub(crate) fn grease_ext(
        ...
        let encoded_inner_hello =
            grease_state.encode_inner_hello(outer_hello, None, &None);
...
impl EchState {
    ...
    pub(crate) fn ech_hello(
        ...
        let encoded_inner_hello =
            self.encode_inner_hello(&outer_hello, retry_req, resuming);
        ...
        outer_hello.encrypted_client_hello =
            Some(outer_hello_ext(self, enc, payload));
    ...
    pub(crate) fn confirm_acceptance(
        ...
        match ConstantTimeEq::ct_eq(
            derived.as_ref(),
            server_hello.random.0[24..].as_ref(),
        )
        .into()
        {
            true => {
    ...
    pub(crate) fn confirm_hrr_acceptance(
        ...
        confirmation_transcript
            .add_message(&Self::hello_retry_request_conf(hrr));
    ...
    pub(crate) fn transcript_hrr_update(
        ...
        let inner_transcript =
            self.inner_hello_transcript.clone().start_hash(hash);
    ...
    fn encode_inner_hello(
        ...
        let mut inner_hello = ClientHelloPayload {
            ...
            // ECH requires TLS 1.3.  A shaped ClientHelloOuter may advertise
            // legacy browser suites, but they must not leak into the inner
            // hello.  The ordinary rustls ECH builder enables TLS 1.3 only,
            // so this filtering is otherwise implicit; custom outer shaping
            // makes it explicit and matches uTLS' separately-built inner.
        ...
        let outer_advertises_tls12 = outer_hello
            .supported_versions
            .as_ref()
            .is_some_and(|versions| versions.tls12)
            || outer_hello.exact_extensions.iter().any(|(extension, _)| {
                *extension == ExtensionType::SupportedVersions
            });
        ...
        for ext in outer_extensions {
            ...
            if ext == ExtensionType::ServerName {
                ...
                if let Some(sni_value) = inner_sni {
                    inner_hello.server_name =
                        Some(ServerNamePayload::from(sni_value));
                }
            ...
            if let Some((_, payload)) = outer_hello
                .exact_extensions
                .iter()
                .find(|(extension, _)| *extension == ext)
            {
                inner_hello.exact_extensions.push((ext, payload.clone()));
            }
            if let Some((_, payload)) = outer_hello
                .raw_extensions
                .iter()
                .find(|(extension, _)| *extension == ext)
            {
                inner_hello.raw_extensions.push((ext, payload.clone()));
            }
        }

        // uTLS builds the protected hello from its TLS 1.3 config while the
        // browser-shaped outer hello may continue to advertise TLS 1.2.  In
        // that case the versions extension cannot be outer-compressed because
        // the two payloads intentionally differ.
        if outer_advertises_tls12 {
            inner_hello.exact_extensions.retain(|(extension, _)| {
                *extension != ExtensionType::SupportedVersions
            });
            inner_hello.supported_versions =
                Some(SupportedProtocolVersions::from_flags(false, true));
        }
    ...
    fn grease_psk(
        &self,
        psk_offer: &mut PresharedKeyOffer,
    ) -> Result<(), Error> {
        ...
        psk_offer.binders = psk_offer
            ...
            .map(|old_binder| {
                ...
                Ok::<PresharedKeyBinder, Error>(PresharedKeyBinder::from(
                    new_binder,
                ))
            })
            .collect::<Result<_, _>>()?;
...
mod tests {
    ...
    fn server_hello_conf_alters_server_hello_random() {
        ...
        let message = EchState::server_hello_conf(
            &server_hello,
            server_hello_encoded_before,
        );
    ...
    fn inner_client_hello_length_conceals_inner_name_length() {
        let base_inner_len =
            inner_hello_encoding_for_name(dns_name_of_len(1), true).len();
        assert!(
            base_inner_len % 32 == 0,
    ...
    fn inner_client_hello_length_does_not_leak_length_of_omitted_inner_name() {
        let base_inner_len =
            inner_hello_encoding_for_name(dns_name_of_len(1), false).len();
        assert!(
            base_inner_len % 32 == 0,
            "inner hello length must be 32-byte padded"
        );
    ...
    impl SecureRandom for FixedRandom {
        fn fill(
            &self,
            buf: &mut [u8],
        ) -> Result<(), crate::rand::GetRandomFailed> {
            buf.fill(0x55);
            Ok(())
        }
    }
