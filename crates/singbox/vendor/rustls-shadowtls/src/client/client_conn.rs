...
pub struct ClientConfig {
    ...
    /// Accept the empty, unknown post-handshake message used by metacubex
    /// REALITY to preserve the cover server's NewSessionTicket record shape.
    ///
    /// This is disabled by default and must only be enabled by a REALITY
    /// compatibility layer.
    pub reality_dummy_ticket_compatibility: bool,
...
impl ClientConfig {
    ...
    pub fn client_hello_customizer(
        &self,
    ) -> Option<&Arc<dyn ClientHelloCustomizer>> {
    ...
    pub(super) fn find_kx_group(
        ...
        self.provider.kx_groups.iter().copied().find(|skxg| {
            skxg.usable_for_version(version) && skxg.name() == group
        })
    }
...
mod connection {
    ...
    impl ClientConnection {
        ...
        pub fn dangerous_into_kernel_connection(
            ...
            (ExtractedSecrets, KernelConnection<ClientConnectionData>),
            Error,
        > {
...
impl fmt::Display for EarlyDataError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::ExceededAllowedEarlyData => {
                f.write_str("cannot send any more early data")
            }
