...
use crate::client::{
    ClientConfig, ClientConnection, Resumption, Tls12Resumption,
};
...
mod tests {
    ...
    use crate::msgs::handshake::{
        CertificateChain, EcParameters, HelloRetryRequestExtensions,
        KeyShareEntry, ServerEcdhParams, ServerExtensions, ServerKeyExchange,
    ...
    fn test_no_session_ticket_request_on_tls_1_3() {
        let mut config = ClientConfig::builder_with_provider(
            super::provider::default_provider().into(),
        )
    ...
    fn cas_extension_in_client_hello_if_server_verifier_requests_it() {
        let cas_sending_server_verifier = ServerVerifierWithAuthorityNames(
            vec![DistinguishedName::from(b"hello".to_vec())],
        );
            ...
            [(&version::TLS12, false), (&version::TLS13, true)]
        {
            let client_hello = client_hello_sent_for_config(
                ...
                .with_custom_certificate_verifier(Arc::new(
                    cas_sending_server_verifier.clone(),
                ))
                .with_no_client_auth(),
            )
            .unwrap();
    ...
    fn test_client_with_custom_verifier_can_accept_ecdsa_sha1_signatures() {
        ...
        let cert = Message {
            version: ProtocolVersion::TLSv1_2,
            payload: MessagePayload::handshake(HandshakeMessagePayload(
                HandshakePayload::Certificate(CertificateChain(vec![
            ...
            payload: MessagePayload::handshake(HandshakeMessagePayload(
                HandshakePayload::ServerKeyExchange(
                    ServerKeyExchangePayload::Known(ServerKeyExchange {
                        ...
                        params: ServerKeyExchangeParams::Ecdh(
                            ServerEcdhParams {
    ...
    fn client_config_for_rpk(key_log: Arc<dyn KeyLog>) -> ClientConfig {
        let mut config =
            ClientConfig::builder_with_provider(x25519_provider().into())
                ...
                .dangerous()
                .with_custom_certificate_verifier(Arc::new(
                    ServerVerifierRequiringRpk,
                ))
                .with_client_cert_resolver(Arc::new(
                    AlwaysResolvesClientRawPublicKeys::new(Arc::new(
    ...
    fn client_key() -> PrivateKeyDer<'static> {
        PrivateKeyDer::from_pem_reader(
            &mut include_bytes!("../../../test-ca/rsa-2048/client.key")
                .as_slice(),
        )
        .unwrap()
    }
    ...
    impl FakeServerCrypto {
        ...
        fn server_handshake_encrypter(&self) -> Box<dyn MessageEncrypter> {
            ...
            // Derive Encrypter
            let key =
                derive_traffic_key(expander.as_ref(), cipher_suite.aead_alg);
...
fn client_hello_sent_for_config(
    config: ClientConfig,
