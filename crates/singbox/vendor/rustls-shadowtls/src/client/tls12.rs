...
use crate::msgs::handshake::{
    CertificateChain, ClientDhParams, ClientEcdhParams,
    ClientKeyExchangeParams, HandshakeMessagePayload, HandshakePayload,
    NewSessionTicketPayload, NewSessionTicketPayloadTls13,
    ServerKeyExchangeParams, SessionId,
};
...
mod server_hello {
    ...
    impl CompleteServerHelloHandling {
        pub(in crate::client) fn handle_server_hello(
            ...
            let has_downgrade_marker =
                self.randoms.server[24..] == tls12::DOWNGRADE_SENTINEL;
            ...
            let must_issue_new_ticket =
                if server_hello.session_ticket_ack.is_some() {
            ...
            let may_send_cert_status =
                server_hello.certificate_status_request_ack.is_some();
...
impl State<ClientConnectionData> for ExpectServerKx<'_> {
        ...
        Self: 'm,
    {
        ...
        let kx =
            opaque_kx.unwrap_given_kxa(self.suite.kx).ok_or_else(|| {
                cx.common.send_fatal_alert(
...
fn emit_client_kx(
    ...
    match kxa {
        KeyExchangeAlgorithm::ECDHE => {
            ClientKeyExchangeParams::Ecdh(ClientEcdhParams {
...
impl State<ClientConnectionData> for ExpectServerDone<'_> {
        ...
        Self: 'm,
    {
        ...
        cx.common.peer_certificates =
            Some(st.server_cert.cert_chain.into_owned());
        ...
        if let Some(client_auth) = &st.client_auth {
            let certs = match client_auth {
                ...
                ClientAuthDetails::Verify { certkey, .. } => {
                    CertificateChain(certkey.cert.clone())
                }
            };
        ...
        let maybe_skxg = match &kx_params {
            ServerKeyExchangeParams::Ecdh(ecdh) => st.config.find_kx_group(
                ecdh.curve_params.named_group,
