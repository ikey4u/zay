...
use crate::client::common::{
    ClientAuthDetails, ClientHelloDetails, ServerCertDetails,
};
use crate::client::ech::{self, EchState, EchStatus};
use crate::client::{hs, ClientConfig, ClientHelloPlan, ClientSessionStore};
use crate::common_state::{
    CommonState, HandshakeFlightTls13, HandshakeKind, KxState, Protocol, Side,
    State,
};
...
use crate::enums::{
    AlertDescription, ContentType, HandshakeType, ProtocolVersion,
    SignatureScheme,
};
...
use crate::msgs::handshake::{
    CertificatePayloadTls13, ClientExtensions, EchConfigPayload,
    HandshakeMessagePayload, HandshakePayload, KeyShareEntry,
    NewSessionTicketPayloadTls13, PresharedKeyBinder, PresharedKeyIdentity,
    PresharedKeyOffer, ServerExtensions, ServerHelloPayload,
...
use crate::tls13::{
    construct_client_verify_message, construct_server_verify_message,
    Tls13CipherSuite,
};
...
pub(super) fn handle_server_hello(
    ...
    let our_key_share =
        KeyExchangeChoice::new(&config, cx, our_key_shares, their_key_share)
            .map_err(|_| {
                cx.common.send_fatal_alert(
...
impl KeyExchangeChoice {
    ...
    fn new(
        ...
        for key_share in our_key_shares.exchanges {
            ...
            let Some((component_group, _)) = key_share.hybrid_component()
            else {
                continue;
            };
...
impl OfferedKeyShares {
    pub(super) fn key_share_entries(
        &self,
    ) -> Result<Vec<KeyShareEntry>, Error> {
        self.offered_groups
            .iter()
            .map(|group| {
...
pub(super) fn initial_key_shares(
    ...
    if let Some(key_share_plan) =
        plan.and_then(|plan| plan.key_share_plan.as_ref())
    {
        return planned_key_shares(
            config,
            kx_state,
            plan,
            key_share_plan.as_slice(),
        );
    }

    let planned_group = match plan
    ...
    let group = planned_group
        .or_else(|| {
            config.resumption.store.kx_hint(server_name).and_then(
                |group_name| {
                    config.find_kx_group(group_name, ProtocolVersion::TLSv1_3)
                },
            )
    ...
    let key_exchange =
        start_key_exchange_for_group(config, group.name(), plan)?;
    *kx_state = KxState::Start(group);
    let mut offered_groups = vec![key_exchange.group()];
    if let Some((component_group, _)) =
        key_exchange.hybrid_component().filter(|(group, _)| {
            config
...
pub(super) fn retry_key_share(
    ...
    let Some(skxg) = config.find_kx_group(group, ProtocolVersion::TLSv1_3)
    else {
        return Err(Error::General(
            "ClientHello key share group is not supported by this config"
                .into(),
        ));
    };
...
fn planned_key_shares(
    ...
    if let Some(supported_groups) =
        plan.and_then(|plan| plan.supported_groups.as_ref())
    {
        for group in groups {
            if !supported_groups.as_slice().contains(group) {
                return Err(Error::General(
    ...
    let first_group = offered
        ...
        .ok_or_else(|| {
            Error::General("ClientHello key share plan cannot be empty".into())
        })?;
...
fn start_key_exchange_for_named_group(
    ...
    let Some(group) = config.find_kx_group(group, ProtocolVersion::TLSv1_3)
    else {
        return Err(Error::General(
            "ClientHello key share group is not supported by this config"
                .into(),
...
fn start_key_exchange_for_group(
    ...
    if matches!(
        ...
        if let Some(fixed_x25519) =
            plan.and_then(|plan| plan.fixed_x25519.as_ref())
        {
    ...
    #[cfg(feature = "aws_lc_rs")]
    {
        if group.name() != NamedGroup::X25519 {
            if core::ptr::eq(group, crypto::aws_lc_rs::kx_group::X25519MLKEM768)
            {
                ...
                if let Some((NamedGroup::X25519, public_key)) =
                    key_exchange.hybrid_component()
                {
                    let public_key: &[u8; 32] =
                        public_key.try_into().map_err(|_| {
                            Error::General(
                                "fixed X25519 public key was not 32 bytes"
                                    .into(),
                            )
                        })?;
        ...
        let key_exchange = crypto::aws_lc_rs::x25519::start_fixed_x25519(
            fixed_x25519.private_key(),
        )?;
        let public_key: &[u8; 32] =
            key_exchange.pub_key().try_into().map_err(|_| {
                Error::General(
                    "fixed X25519 public key was not 32 bytes".into(),
                )
            })?;
        if let Some(observer) = fixed_x25519.observer() {
            observer.observe_x25519_key_share(public_key)?;
...
pub(super) fn fill_in_psk_binder(
    ...
    if let HandshakePayload::ClientHello(ch) = &mut hmp.0 {
        ...
        }) = &mut ch.preshared_key_offer
        {
            ...
            debug_assert_eq!(
                binders[0].as_ref().len(),
                real_binder.as_ref().len()
            );
            binders[0] =
                PresharedKeyBinder::from(real_binder.as_ref().to_vec());
        }
    };
...
fn validate_encrypted_extensions(
    ...
    if exts.contains_any(ALLOWED_PLAINTEXT_EXTS)
        || exts.contains_any(DISALLOWED_TLS13_EXTS)
    {
        return Err(common.send_fatal_alert(
            AlertDescription::UnsupportedExtension,
...
impl State<ClientConnectionData> for ExpectEncryptedExtensions {
        ...
        Self: 'm,
    {
        ...
        let ech_retry_configs =
            match (cx.data.ech_status, &exts.encrypted_client_hello_ack) {
                ...
                // of the handshake.
                (EchStatus::Rejected, ext) => {
                    ext.as_ref().map(|ext| ext.retry_configs.to_vec())
                }
                _ => None,
            };
        ...
        if cx.common.is_quic() {
            ...
            {
                Some(params) => {
                    cx.common.quic.params = Some(params.clone().into_vec())
                }
                None => {
                    return Err(cx.common.missing_extension(
        ...
        match self.resuming_session {
            Some(resuming_session) => {
                ...
                cx.common.peer_certificates =
                    Some(resuming_session.server_cert_chain().clone());
...
impl State<ClientConnectionData>
    for ExpectCertificateOrCompressedCertificateOrCertReq
{
    fn handle<'m>(
        self: Box<Self>,
        cx: &mut ClientContext<'_>,
...
impl State<ClientConnectionData> for ExpectCertificateRequest {
        ...
        Self: 'm,
    {
        ...
        let compat_compressor = certreq
            ...
            .as_deref()
            .and_then(|offered| {
                self.config.cert_compressors.iter().find(|compressor| {
                    offered.contains(&compressor.algorithm())
                })
            })
            .cloned();
...
impl State<ClientConnectionData> for ExpectCompressedCertificate {
        ...
        Self: 'm,
    {
        ...
        if compressed_cert.uncompressed_len as usize
            > CERTIFICATE_MAX_SIZE_LIMIT
        {
            return Err(cx.common.send_fatal_alert(
                AlertDescription::BadCertificate,
        ...
        let mut decompress_buffer =
            vec![0u8; compressed_cert.uncompressed_len as usize];
        if let Err(compress::DecompressionFailed) = decompressor.decompress(
        ...
        let cert_payload = match CertificatePayloadTls13::read(
            &mut Reader::init(&decompress_buffer),
        ) {
            Ok(cm) => cm,
            Err(err) => {
                return Err(cx
...
impl State<ClientConnectionData> for ExpectCertificateVerify<'_> {
        ...
        Self: 'm,
    {
        ...
        cx.common.peer_certificates =
            Some(self.server_cert.cert_chain.into_owned());
        self.transcript.add_message(&m);
...
fn emit_compressed_certificate_tls13(
    ...
    let mut cert_payload =
        CertificatePayloadTls13::new(certkey.cert.iter(), None);
    cert_payload.context =
        PayloadU8::new(auth_context.clone().unwrap_or_default());
...
fn emit_certverify_tls13(
    ...
    let message =
        construct_client_verify_message(&flight.transcript.current_hash());
...
impl State<ClientConnectionData> for ExpectFinished {
        ...
        Self: 'm,
    {
        ...
        let expect_verify_data =
            st.key_schedule.sign_server_finish(&handshake_hash);

        let fin = match ConstantTimeEq::ct_eq(
        ...
        cx.common.check_aligned_handshake()?;
        let (key_schedule, resumption) = key_schedule_pre_finished
            .into_traffic(cx.common, st.transcript.current_hash());
...
impl ExpectTraffic {
    ...
    fn handle_new_ticket_tls13(
        ...
        cx.common.tls13_tickets_received =
            cx.common.tls13_tickets_received.saturating_add(1);
...
impl State<ClientConnectionData> for ExpectTraffic {
        ...
        Self: 'm,
    {
        match m.payload {
            ...
            MessagePayload::Handshake {
                parsed:
                    HandshakeMessagePayload(HandshakePayload::KeyUpdate(key_update)),
                ..
            } => self.handle_key_update(cx.common, &key_update)?,
            MessagePayload::Handshake {
                parsed:
                    HandshakeMessagePayload(HandshakePayload::Unknown((
                        HandshakeType::Unknown(0x17),
                        _,
                    ))),
                ..
            } if self.config.reality_dummy_ticket_compatibility => {}
            payload => {
...
impl State<ClientConnectionData> for ExpectQuicTraffic {
    ...
    fn export_keying_material(
        ...
        State::export_keying_material(&self.0, output, label, context)
    }
