...
use crate::crypto::cipher::{
    InboundOpaqueMessage, MessageDecrypter, MessageEncrypter,
};
...
impl RecordLayer {
    ...
    pub(crate) fn decrypt_incoming<'a>(
        ...
        match self.message_decrypter.decrypt(encr, self.read_seq) {
            ...
            Err(Error::DecryptError)
                if self.doing_trial_decryption(encrypted_len) =>
            {
                trace!(
                    "Dropping undecryptable message after aborted early_data"
                );
                Ok(None)
            }
            Err(err) => Err(err),
    ...
    pub(crate) fn encrypt_outgoing_with_padding(
        &mut self,
        plain: OutboundPlainMessage<'_>,
        padding_len: usize,
    ) -> OutboundOpaqueMessage {
        ...
        self.message_encrypter
            .encrypt_with_padding(plain, seq, padding_len)
            .unwrap()
    }
