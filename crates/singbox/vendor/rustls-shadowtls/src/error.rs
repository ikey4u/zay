...
impl Error {
    /// Return the ECHConfigList supplied by a server that rejected ECH.
    ///
    /// The returned bytes include the two-byte ECHConfigList length prefix and
    /// can be passed directly to [`crate::client::EchConfig::new`].
    pub fn ech_retry_config_list(&self) -> Option<Vec<u8>> {
        match self {
            Self::PeerIncompatible(
                PeerIncompatible::ServerRejectedEncryptedClientHello(Some(
                    configs,
                )),
            ) if !configs.is_empty() => Some(configs.get_encoding()),
            _ => None,
        }
    }
}
        ...
        match (self, other) {
            ...
            (
                ...
                (left_signature_algorithm_id, left_supported_algorithms)
                    == (
                        right_signature_algorithm_id,
                        right_supported_algorithms,
                    )
            }
            (
                UnsupportedSignatureAlgorithmForPublicKeyContext {
            ...
            (
                ...
                (left_expected, left_presented)
                    == (right_expected, right_presented)
            }
            ...
            (
                ...
                (left_required, left_presented)
                    == (right_required, right_presented)
            }
...
impl From<CertificateError> for AlertDescription {
    fn from(e: CertificateError) -> Self {
        ...
        match e {
            ...
            | ExpiredContext { .. }
            | NotValidYet
            | NotValidYetContext { .. } => Self::CertificateExpired,
...
impl PartialEq<Self> for CertRevocationListError {
    fn eq(&self, other: &Self) -> bool {
        ...
        match (self, other) {
            ...
            (UnsupportedCriticalExtension, UnsupportedCriticalExtension) => {
                true
            }
...
mod other_error {
    ...
    pub struct OtherError(
        #[cfg(feature = "std")] pub Arc<dyn StdError + Send + Sync>,
    );
...
mod tests {
    ...
    use super::{
        CertRevocationListError, Error, InconsistentKeys, InvalidMessage,
        OtherError, UnixTime,
    };
    #[cfg(feature = "std")]
    use crate::sync::Arc;
    ...
    fn certificate_error_equality() {
        ...
        assert_ne!(
            ...
            NotValidYetContext {
                time: UnixTime::since_unix_epoch(Duration::from_secs(1234)),
                not_before: UnixTime::since_unix_epoch(Duration::from_secs(
                    1234
                )),
            }
        );
        ...
        assert_ne!(
            ...
            ExpiredRevocationListContext {
                time: UnixTime::since_unix_epoch(Duration::from_secs(12345)),
                next_update: UnixTime::since_unix_epoch(Duration::from_secs(
                    123
                )),
            }
        );
