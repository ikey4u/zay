...
use h3_datagram::quic_traits::{
    DatagramConnectionExt, RecvDatagram, SendDatagram,
...
pub struct RecvDatagramHandler {
    datagrams:
        BoxStreamSync<'static, <ReadDatagram<'static> as Future>::Output>,
}
...
impl<B: Buf> DatagramConnectionExt<B> for Connection {
    ...
    fn recv_datagram_handler(&self) -> Self::RecvDatagramHandler {
        RecvDatagramHandler {
            datagrams: Box::pin(stream::unfold(
                self.conn.clone(),
                |conn| async { Some((conn.read_datagram().await, conn)) },
            )),
        }
    }
}

fn convert_send_datagram_error(
    ...
    match error {
        ...
        SendDatagramError::ConnectionLost(e) => {
            SendDatagramErrorIncoming::ConnectionError(
...
fn convert_h3_error_to_datagram_error(
    ...
    match error {
        ...
        ConnectionErrorIncoming::Timeout => {
            h3_datagram::ConnectionErrorIncoming::Timeout
        }
