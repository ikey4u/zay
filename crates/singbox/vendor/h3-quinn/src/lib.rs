...
use h3::{
    error::Code,
    quic::{
        self, ConnectionErrorIncoming, StreamErrorIncoming, StreamId, WriteBuf,
    },
};
...
pub struct Connection {
    ...
    opening_bi:
        Option<BoxStreamSync<'static, <OpenBi<'static> as Future>::Output>>,
    incoming_uni:
        BoxStreamSync<'static, <AcceptUni<'static> as Future>::Output>,
    opening_uni:
        Option<BoxStreamSync<'static, <OpenUni<'static> as Future>::Output>>,
}
...
impl Connection {
    ...
    pub fn new(conn: quinn::Connection) -> Self {
        Self {
            ...
            opening_bi: None,
            incoming_uni: Box::pin(stream::unfold(
                conn.clone(),
                |conn| async { Some((conn.accept_uni().await, conn)) },
            )),
            opening_uni: None,
        }
    }
...
fn convert_read_error_to_stream_error(error: ReadError) -> StreamErrorIncoming {
    match error {
        ...
        ReadError::IllegalOrderedRead => {
            panic!("h3-quinn only performs ordered reads")
        }
        error @ ReadError::ZeroRttRejected => {
            StreamErrorIncoming::Unknown(Box::new(error))
        }
    }
}

fn convert_write_error_to_stream_error(
    error: quinn::WriteError,
) -> StreamErrorIncoming {
    match error {
        quinn::WriteError::Stopped(var_int) => {
        ...
        error @ quinn::WriteError::ClosedStream
        | error @ quinn::WriteError::ZeroRttRejected => {
            StreamErrorIncoming::Unknown(Box::new(error))
        }
    }
}
