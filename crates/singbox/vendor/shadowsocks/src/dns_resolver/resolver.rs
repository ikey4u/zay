...
impl Debug for DnsResolver {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            #[cfg(feature = "hickory-dns")]
            Self::HickoryDnsSystem { .. } => {
                f.write_str("HickoryDnsSystem(..)")
            }
            #[cfg(feature = "hickory-dns")]
...
#[cfg(all(feature = "hickory-dns", unix, not(target_os = "android")))]
async fn hickory_dns_notify_update_dns(
    resolver: Arc<HickoryDnsSystemResolver>,
) -> notify::Result<()> {
    ...
    let mut watcher: RecommendedWatcher =
        notify::recommended_watcher(move |ev_result: NotifyResult<Event>| {
            match ev_result {
                Ok(ev) => {
    ...
    watcher
        .watch(Path::new(DNS_RESOLV_FILE_PATH), RecursiveMode::NonRecursive)?;
        ...
        let task = {
            ...
            tokio::spawn(async move {
                ...
                .await
                {
                    ...
                    Err(err) => {
                        error!(
                            "failed to reload {DNS_RESOLV_FILE_PATH}, error: {err}"
                        );
                    }
                }
            })
...
impl DnsResolver {
    ...
    pub async fn hickory_dns_system_resolver(
        ...
        let resolver =
            create_resolver(None, opts.clone(), connect_opts.clone()).await?;
                ...
                match self.start_time {
                    Some(start_time) => {
                        ...
                        match *self.resolver {
                            ...
                            #[cfg(feature = "hickory-dns")]
                            DnsResolver::HickoryDnsSystem { .. }
                            | DnsResolver::HickoryDns(..) => {
                                trace!(
                    ...
                    None => match *self.resolver {
                        ...
                        | DnsResolver::HickoryDns(..) => {
                            trace!(
                                "DNS resolved {}:{} with hickory-dns",
                                self.addr, self.port
                            );
                        }
                        DnsResolver::Custom(..) => {
                            trace!(
                                "DNS resolved {}:{} with customized",
                                self.addr, self.port
                            );
                        }
                    },
            ...
            Self::System => match lookup_host((addr, port)).await {
                ...
                Err(err) => {
                    let err = Error::other(format!(
                        "dns resolve {addr}:{port} error: {err}"
                    ));
                    Err(err)
                }
            },
            #[cfg(feature = "hickory-dns")]
            Self::HickoryDnsSystem { ref inner, .. } => {
                match inner.resolver.load().lookup_ip(addr).await {
                    Ok(lookup_result) => {
                        let result: Vec<SocketAddr> = lookup_result
                            .iter()
