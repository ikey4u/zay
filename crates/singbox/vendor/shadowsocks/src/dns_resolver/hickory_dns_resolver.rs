...
use crate::net::{
    ConnectOpts, tcp::TcpStream as ShadowTcpStream,
    udp::UdpSocket as ShadowUdpSocket,
};
...
impl RuntimeProvider for ShadowDnsRuntimeProvider {
    ...
    fn bind_udp(
        ...
        Box::pin(async move {
            ...
            let udp =
                ShadowUdpSocket::bind_with_opts(&local_addr, &connect_opts)
                    .await?;
            trace!(
    ...
    match dns {
        Some(conf) => {
            ...
            let mut builder = DnsResolver::builder_with_config(
                conf,
                ShadowDnsRuntimeProvider::new(connect_opts),
            );
        ...
        None => {
            match DnsResolver::builder(ShadowDnsRuntimeProvider::new(
                connect_opts,
            )) {
                Ok(mut builder) => {
