...
const USER_KEY_BASE64_ENGINE: base64::engine::GeneralPurpose =
...
const AEAD2022_PASSWORD_BASE64_ENGINE: base64::engine::GeneralPurpose =
    base64::engine::GeneralPurpose::new(
        &base64::alphabet::STANDARD,
        base64::engine::GeneralPurposeConfig::new()
            .with_encode_padding(true)
...
const URL_PASSWORD_BASE64_ENGINE: base64::engine::GeneralPurpose =
    base64::engine::GeneralPurpose::new(
...
impl ServerUser {
    ...
    pub fn with_encoded_key<N>(
        name: N,
        key: &str,
    ) -> Result<Self, ServerUserError>
    where
        N: Into<String>,
    {
...
impl ServerUserManager {
    ...
    pub fn add_user(&mut self, user: ServerUser) {
        self.users
            .insert(user.clone_identity_hash(), Arc::new(user));
    }
    ...
    pub fn clone_user_by_hash(
        &self,
        user_hash: &[u8],
    ) -> Option<Arc<ServerUser>> {
...
pub enum ServerConfigError {
    ...
    /// Key length mismatch
    #[error(
        "invalid key length for {0}, expecting {1} bytes, but found {2} bytes"
    )]
    InvalidKeyLength(CipherKind, usize, usize),

    /// User Key (ipsk) length mismatch
    #[error(
        "invalid user key length for {0}, expecting {1} bytes, but found {2} bytes"
    )]
        ...
        match AEAD2022_PASSWORD_BASE64_ENGINE.decode(password) {
            Ok(v) => {
                if v.len() != enc_key.len() {
                    return Err(ServerConfigError::InvalidKeyLength(
                        method,
                        enc_key.len(),
                        v.len(),
                    ));
...
fn password_to_keys<P>(
    method: CipherKind,
    password: P,
) -> Result<(String, Box<[u8]>, Vec<Bytes>), ServerConfigError>
where
    P: Into<String>,
{
    ...
    if method_support_eih(method) {
        ...
        for ipsk in split_iter {
            match USER_KEY_BASE64_ENGINE.decode(ipsk) {
                Ok(v) => {
                    ...
                    match method {
                        CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                            ...
                            if v.len() != 16 {
                                return Err(
                                    ServerConfigError::InvalidUserKeyLength(
                                        method,
                                        16,
                                        v.len(),
                                    ),
                ...
                Err(err) => {
                    return Err(ServerConfigError::InvalidUserKeyEncoding(
                        method, err,
                    ));
                }
            }
...
impl ServerConfig {
    ...
    where
        ...
        P: Into<String>,
    {
        let (password, enc_key, identity_keys) =
            password_to_keys(method, password)?;
    ...
    pub fn to_qrcode_url(&self) -> String {
        let param =
            format!("{}:{}@{}", self.method(), self.password(), self.addr());
    ...
    pub fn to_url(&self) -> String {
        ...
        if let Some(c) = self.plugin() {
            ...
            url += "/?plugin=";
            for c in percent_encoding::utf8_percent_encode(
                &plugin,
    ...
    pub fn from_url(encoded: &str) -> Result<Self, UrlParseError> {
        ...
        if user_info.is_empty() {
            ...
            let mut decoded_body = match URL_PASSWORD_BASE64_ENGINE
                .decode(encoded)
            {
        ...
        let (method, pwd) = match parsed.password() {
            Some(password) => {
                ...
                let m = match percent_encoding::percent_decode_str(user_info)
                    .decode_utf8()
                {
                    Ok(m) => m,
                    Err(err) => {
                ...
                let p = match percent_encoding::percent_decode_str(password)
                    .decode_utf8()
                {
                    Ok(m) => m,
                    Err(err) => {
            ...
            None => {
                ...
                let decoded_user_info =
                    match percent_encoding::percent_decode_str(user_info)
                        .decode_utf8()
                    {
                        Ok(m) => m,
                        Err(err) => {
                            error!(
                                "failed to parse percent-encoded userinfo, err: {}",
                                err
                            );
                            return Err(UrlParseError::InvalidAuthInfo);
                        }
                ...
                let account = match URL_PASSWORD_BASE64_ENGINE
                    .decode(decoded_user_info)
                {
        ...
        let method = match method.parse::<CipherKind>() {
            ...
            Err(err) => {
                error!(
                    "failed to parse \"{}\" to CipherKind, err: {:?}",
                    method, err
                );
        ...
        if let Some(q) = parsed.query() {
            let query = match serde_urlencoded::from_bytes::<
                Vec<(String, String)>,
            >(q.as_bytes())
            {
                Ok(q) => q,
                ...
                match (sp.next(), sp.next()) {
                    (Some(dn), Some(port)) => {
                        ...
                        match port.parse::<u16>() {
                            Ok(port) => {
                                Ok(Self::DomainName(dn.to_owned(), port))
                            }
...
impl From<&Address> for ServerAddr {
    fn from(addr: &Address) -> Self {
        match *addr {
            Address::SocketAddress(sa) => Self::SocketAddr(sa),
            Address::DomainNameAddress(ref dn, port) => {
                Self::DomainName(dn.clone(), port)
            }
        }
    }
}
...
impl From<&ServerAddr> for Address {
    fn from(addr: &ServerAddr) -> Self {
        match *addr {
            ServerAddr::SocketAddr(sa) => Self::SocketAddress(sa),
            ServerAddr::DomainName(ref dn, port) => {
                Self::DomainNameAddress(dn.clone(), port)
            }
        }
    }
}
            ...
            Some(pos) => {
                ...
                match s.parse::<SocketAddr>() {
                    ...
                    Err(..) => {
                        ...
                        let (sdomain, sport) =
                            (sdomain.trim(), sport[1..].trim());

                        match sport.parse::<u16>() {
                            Ok(port) => {
                                Ok(Self::DomainName(sdomain.to_owned(), port))
                            }
...
mod test {
    ...
    fn test_server_config_from_url() {
        let server_config =
            ServerConfig::from_url("ss://foo:bar@127.0.0.1:9999");
