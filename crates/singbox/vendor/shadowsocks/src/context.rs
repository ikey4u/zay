...
pub struct Context {
    ...
    // Injectable wall clock for AEAD-2022 timestamp generation and checks.
    time_provider: Arc<dyn TimeProvider>,
}

/// Wall clock used by AEAD-2022 protocol timestamp fields.
///
/// Embedders can supply an NTP-corrected implementation without changing the
/// process-wide system clock. Implementations must be cheap and non-blocking.
...
impl Context {
        ...
        match replay_policy {
            ...
            ReplayAttackPolicy::Detect => {
                if self.replay_protector.check_nonce_and_set(method, nonce) {
                    warn!(
                        "detected repeated nonce (iv/salt) {:?}",
                        ByteStr::new(nonce)
                    );
                }
                Ok(())
            ...
            ReplayAttackPolicy::Reject => {
                if self.replay_protector.check_nonce_and_set(method, nonce) {
                    let err =
                        io::Error::other("detected repeated nonce (iv/salt)");
                    Err(err)
                } else {
                    Ok(())
                }
    ...
    pub fn set_replay_attack_policy(
        &mut self,
        replay_policy: ReplayAttackPolicy,
    ) {
    ...
    /// Replace the wall clock used by AEAD-2022 timestamps.
    pub fn set_time_provider(&mut self, provider: Arc<dyn TimeProvider>) {
        self.time_provider = provider;
    }

    /// Read the current wall clock used by AEAD-2022 timestamps.
    pub fn now(&self) -> SystemTime {
        self.time_provider.now()
    }

    /// Clone the configured provider for protocol writers that outlive a
    /// borrowed `Context` reference.
