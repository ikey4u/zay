...
impl ManagerDatagram {
    ...
    pub async fn bind(
        ...
        match *bind_addr {
            ...
            ManagerAddr::DomainName(ref dname, port) => {
                let (_, socket) =
                    lookup_then!(context, dname, port, |saddr| {
                        ShadowUdpSocket::listen(&saddr).await
                    })?;
    ...
    pub async fn connect(
        ...
        match *bind_addr {
            ManagerAddr::SocketAddr(sa) => {
                Self::connect_socket_addr(sa, connect_opts).await
            }
    ...
    async fn connect_socket_addr(
        ...
        let socket =
            ShadowUdpSocket::connect_with_opts(&sa, connect_opts).await?;
        Ok(Self::UdpDatagram(socket.into()))
    }
    ...
    pub async fn recv_from(
        &mut self,
        buf: &mut [u8],
    ) -> io::Result<(usize, ManagerSocketAddr)> {
        match *self {
    ...
    pub async fn send_to(
        ...
        match *self {
            Self::UdpDatagram(ref mut udp) => match *target {
                ManagerSocketAddr::SocketAddr(ref saddr) => {
                ...
                ManagerSocketAddr::UnixSocketAddr(..) => {
                    let err = io::Error::new(
                        ...
                        "udp datagram requires IP address target",
                    );
                    Err(err)
                }
            },
            ...
            Self::UnixDatagram(ref mut unix) => match *target {
                ManagerSocketAddr::UnixSocketAddr(ref saddr) => {
                    match saddr.as_pathname() {
                ...
                ManagerSocketAddr::SocketAddr(..) => {
                    let err = io::Error::new(
                        ErrorKind::InvalidInput,
                        "unix datagram requires path address target",
                    );
                    Err(err)
                }
            },
    ...
    pub async fn send_to_manager(
        ...
        match *self {
            Self::UdpDatagram(ref mut udp) => match *target {
                ...
                ManagerAddr::DomainName(ref dname, port) => {
                    let (_, n) = lookup_then!(context, dname, port, |saddr| {
                        udp.send_to(buf, saddr).await
                    })?;
                    Ok(n)
                }
            ...
            #[cfg(unix)]
            Self::UnixDatagram(ref mut unix) => match *target {
                ManagerAddr::UnixSocketAddr(ref paddr) => {
                    unix.send_to(buf, paddr).await
                }
    ...
    pub fn local_addr(&self) -> io::Result<ManagerSocketAddr> {
        match *self {
            Self::UdpDatagram(ref socket) => {
                socket.local_addr().map(ManagerSocketAddr::SocketAddr)
            }
            #[cfg(unix)]
            Self::UnixDatagram(ref dgram) => {
                dgram.local_addr().map(ManagerSocketAddr::UnixSocketAddr)
            }
        }
    }
}
