...
use super::{
    ...
    sys::{
        bind_outbound_udp_socket, create_inbound_udp_socket,
        create_outbound_udp_socket,
    },
};
...
impl UdpSocket {
    ...
    pub async fn connect_server_with_opts(
        ...
        let socket = match *addr {
            ServerAddr::SocketAddr(ref remote_addr) => {
                let socket =
                    create_outbound_udp_socket(From::from(remote_addr), opts)
                        .await?;
            ...
            ServerAddr::DomainName(ref dname, port) => {
                lookup_then!(context, dname, port, |remote_addr| {
                    let s = create_outbound_udp_socket(
    ...
    /// Binds to a specific address (inbound)
    pub async fn listen_with_opts(
        addr: &SocketAddr,
        opts: AcceptOpts,
    ) -> io::Result<Self> {
        let socket = create_inbound_udp_socket(
            addr,
            opts.ipv6_only,
            opts.udp.allow_fragmentation,
        )
        .await?;
        Ok(Self {
            socket,
    ...
    pub fn poll_batch_send(
        ...
        loop {
            ...
            match self.socket.try_io(Interest::WRITABLE, || {
                batch_sendmsg(&self.socket, msgs)
            }) {
    ...
    pub fn poll_batch_recv(
        ...
        loop {
            ...
            match self.socket.try_io(Interest::READABLE, || {
                batch_recvmsg(&self.socket, msgs)
            }) {
                Ok(n) => return Ok(n).into(),
                Err(ref err) if err.kind() == ErrorKind::WouldBlock => {}
                Err(err) => return Err(err).into(),
