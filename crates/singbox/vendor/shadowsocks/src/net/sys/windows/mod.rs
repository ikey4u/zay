...
use socket2::{
    Domain, Protocol, SockAddr, SockRef, Socket, TcpKeepalive, Type,
};
use tokio::{
...
use windows_sys::{
    Win32::{
        Foundation::{
            ERROR_BUFFER_OVERFLOW, ERROR_NO_DATA, ERROR_SUCCESS, FALSE,
        },
        ...
        Networking::WinSock::{
            AF_UNSPEC, IP_MTU_DISCOVER, IP_PMTUDISC_DO, IP_UNICAST_IF,
            IPPROTO_IP, IPPROTO_IPV6, IPPROTO_TCP, IPV6_MTU_DISCOVER,
            IPV6_UNICAST_IF, SIO_UDP_CONNRESET, SOCKET, SOCKET_ERROR,
            TCP_FASTOPEN, WSAGetLastError, WSAIoctl, htonl, setsockopt,
        },
    },
    core::{BOOL, PCSTR},
};
...
pub async fn create_inbound_tcp_socket(
    bind_addr: &SocketAddr,
    _accept_opts: &AcceptOpts,
) -> io::Result<TcpSocket> {
    match bind_addr {
        SocketAddr::V4(..) => TcpSocket::new_v4(),
        SocketAddr::V6(..) => TcpSocket::new_v6(),
...
fn find_adapter_interface_index(
    ...
    unsafe {
        let mut ip_adapter_addresses_buffer =
            BytesMut::with_capacity(15 * 1024);
        ...
        while !current_ip_adapter_address.is_null() {
            ...
            let friendly_name_slice: &[u16] = slice::from_raw_parts(
                ...
                && friendly_name == iface
            {
                match ip {
                    ...
                    IpAddr::V6(..) => {
                        return Ok(Some(ip_adapter_address.Ipv6IfIndex));
                    }
                }
            }

            // Adapter Name
            let adapter_name = CStr::from_ptr(
...
fn set_ip_unicast_if<S: AsRawSocket>(
    socket: &S,
    addr: &SocketAddr,
    iface: &str,
) -> io::Result<()> {
...
#[inline]
pub fn set_disable_ip_fragmentation<S: AsRawSocket>(
    af: AddrFamily,
    socket: &S,
) -> io::Result<()> {
    let handle = socket.as_raw_socket() as SOCKET;
...
pub async fn create_inbound_udp_socket(
    ...
    if !allow_fragmentation
        && let Err(err) = set_disable_ip_fragmentation(addr_family, &socket)
    {
...
pub async fn bind_outbound_udp_socket(
    bind_addr: &SocketAddr,
    opts: &ConnectOpts,
) -> io::Result<UdpSocket> {
...
pub fn set_common_sockopt_after_connect<S>(
    stream: &S,
    opts: &ConnectOpts,
) -> io::Result<()>
where
...
fn set_common_sockopt_after_connect_impl(
    socket: &Socket,
    opts: &ConnectOpts,
) -> io::Result<()> {
    if opts.tcp.nodelay {
...
pub fn set_common_sockopt_after_accept<S>(
    stream: &S,
    opts: &AcceptOpts,
) -> io::Result<()>
where
...
{
    socket_call_warp(stream, |socket| {
        set_common_sockopt_after_accept_impl(socket, opts)
    })
}

fn set_common_sockopt_after_accept_impl(
    socket: &Socket,
    opts: &AcceptOpts,
) -> io::Result<()> {
