...
use crate::net::{
    ...
    sys::{
        set_common_sockopt_after_connect, set_common_sockopt_for_connect,
...
#[inline]
pub fn set_disable_ip_fragmentation<S: AsRawFd>(
    af: AddrFamily,
    socket: &S,
) -> io::Result<()> {
...
mod android {
    ...
    async fn send_vpn_protect_uds<P: AsRef<Path>>(
        protect_path: P,
        fd: RawFd,
    ) -> io::Result<()> {
        ...
        S: AsRawFd + Send + Sync + 'static,
    {
        ...
        if let Some(ref path) = opts.vpn_protect_path {
            ...
            match time::timeout(
                ...
                send_vpn_protect_uds(path, socket.as_raw_fd()),
            )
            .await
            {
                Ok(Ok(..)) => {}
...
fn recvmsg_fallback<S: AsRawFd>(
    sock: &S,
    msg: &mut BatchRecvMessage<'_>,
) -> io::Result<()> {
...
pub fn batch_recvmsg<S: AsRawFd>(
    sock: &S,
    ...
    if ret < 0 {
        ...
        if let Some(libc::ENOSYS) = err.raw_os_error() {
            debug!(
                "recvmmsg is not supported, fallback to recvmsg, error: {:?}",
                err
            );
...
fn sendmsg_fallback<S: AsRawFd>(
    sock: &S,
    msg: &mut BatchSendMessage<'_>,
) -> io::Result<()> {
    let mut hdr: libc::msghdr = unsafe { mem::zeroed() };
...
pub fn batch_sendmsg<S: AsRawFd>(
    sock: &S,
    ...
    if ret < 0 {
        ...
        if let Some(libc::ENOSYS) = err.raw_os_error() {
            debug!(
                "sendmmsg is not supported, fallback to sendmsg, error: {:?}",
                err
            );
            SUPPORT_BATCH_SEND_RECV_MSG.store(false, Ordering::Relaxed);
