        ...
        match (baddr, addr) {
            ...
            (SocketAddr::V4(v4baddr), SocketAddr::V6(..)) => {
                socket.bind(SocketAddr::new(
                    v4baddr.ip().to_ipv6_mapped().into(),
                    v4baddr.port(),
                ))?;
            }
            ...
            (SocketAddr::V6(v6baddr), SocketAddr::V4(..)) => match v6baddr
                .ip()
                .to_ipv4_mapped()
            {
                Some(v4baddr) => socket
                    .bind(SocketAddr::new(v4baddr.into(), v6baddr.port()))?,
                None => {
...
#[cfg(unix)]
pub fn socket_bind_dual_stack<S>(
    socket: &S,
    addr: &SocketAddr,
    ipv6_only: bool,
) -> io::Result<()>
        ...
        match socket.bind(&saddr) {
            ...
            Err(ref err) if err.kind() == ErrorKind::AddrInUse => {
                ...
                if let Err(err) = socket.set_only_v6(true) {
                    warn!(
                        "failed to set IPV6_V6ONLY: true for socket, error: {}",
                        err
                    );
...
static IP_STACK_CAPABILITIES: LazyLock<IpStackCapabilities> =
    LazyLock::new(|| {
        ...
        {
            let local_host =
                SockAddr::from(SocketAddr::new(Ipv6Addr::LOCALHOST.into(), 0));
...
fn check_ipv4_mapped_ipv6_capability() -> io::Result<()> {
    ...
    let local_host = SockAddr::from(SocketAddr::new(
