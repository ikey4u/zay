...
static DEFAULT_ACCEPT_OPTS: LazyLock<AcceptOpts> =
...
impl ProxyListener {
    ...
    pub async fn bind(
        ...
        Self::bind_with_opts(context, svr_cfg, DEFAULT_ACCEPT_OPTS.clone())
            .await
    }
    ...
    pub async fn bind_with_opts(
        ...
        let listener = match svr_cfg.tcp_external_addr() {
            ServerAddr::SocketAddr(sa) => {
                TcpListener::bind_with_opts(sa, accept_opts).await?
            }
            ServerAddr::DomainName(domain, port) => {
                lookup_then!(&context, domain, *port, |addr| {
                    TcpListener::bind_with_opts(&addr, accept_opts.clone())
                        .await
                })?
                .1
            }
    ...
    pub async fn accept(
        &self,
    ) -> io::Result<(ProxyServerStream<TcpStream>, SocketAddr)> {
    ...
    pub async fn accept_map<F, S>(
        &self,
        map_fn: F,
    ) -> io::Result<(ProxyServerStream<S>, SocketAddr)>
    where
