...
use super::{
    crypto_io::StreamType,
    proxy_stream::protocol::v2::SERVER_STREAM_TIMESTAMP_MAX_DIFF,
};
use crate::{
    config::{ServerUserManager, method_support_eih},
    context::{Context, TimeProvider},
    crypto::{CipherKind, v2::tcp::TcpCipher},
};

#[inline]
fn get_now_timestamp(provider: &dyn TimeProvider) -> u64 {
    match provider
        .now()
...
impl DecryptedReader {
        ...
        loop {
            match self.state {
                DecryptReadState::ReadHeader { ref key } => {
                    ...
                    match ready!(
                        self.poll_read_header(cx, context, stream, key)
                    )? {
                        None => {
                            return Ok(()).into();
                        }
                ...
                DecryptReadState::ReadLength => {
                    match ready!(self.poll_read_length(cx, stream))? {
                        None => {
                            return Ok(()).into();
                        }
                        Some(length) => {
                ...
                DecryptReadState::ReadData { length } => {
                    ...
                    self.data_chunk_count =
                        self.data_chunk_count.wrapping_add(1);
                }
                DecryptReadState::BufferedData { ref mut pos } => {
                    if *pos < self.buffer.len() {
                        ...
                        let consumed =
                            usize::min(buffered.len(), buf.remaining());
        ...
        S: AsyncRead + Unpin + ?Sized,
    {
        ...
        let require_eih = self.stream_ty == StreamType::Server
            && method_support_eih(self.method)
            && self.user_manager.is_some();
        let eih_len = if require_eih { 16 } else { 0 };
        let header_len = salt_len
            + eih_len
            + 1
            + 8
            + request_salt_len
            ...
            match self.user_manager {
                Some(ref user_manager) => {
                    ...
                    if header_chunk.len() < 16 {
                        error!(
                            "expecting EIH, but header chunk len: {}",
                            header_chunk.len()
                        );
                        return Err(
                            ProtocolError::MissingExtendedIdentityHeader,
                        )
                        .into();
                    }

                    let (eih, remain_header_chunk) =
                        header_chunk.split_at_mut(16);
                    header_chunk = remain_header_chunk;

                    let key_material = [key, salt].concat();
                    let identity_sub_key = blake3::derive_key(
                    ...
                    match self.method {
                        CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                            let cipher = Aes128::new_from_slice(
                                &identity_sub_key[0..16],
                            )
                            .expect("AES-128");
                            cipher.decrypt_block_b2b(
                                <&Block as TryFrom<&[u8]>>::try_from(eih)
                                    .expect("EIH key length mismatch"),
                                &mut user_hash,
                            );
                        }
                        CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
                            let cipher = Aes256::new_from_slice(
                                &identity_sub_key[0..32],
                            )
                            .expect("AES-256");
                            cipher.decrypt_block_b2b(
                                <&Block as TryFrom<&[u8]>>::try_from(eih)
                        ...
                        _ => {
                            unreachable!("{} doesn't support EIH", self.method)
                        }
                    }
                    ...
                    match user_manager.get_user_by_hash(user_hash) {
                        None => {
                            return Err(ProtocolError::InvalidClientUser(
                                Bytes::copy_from_slice(user_hash),
                            ))
                            .into();
                        }
                        Some(user) => {
                            ...
                            self.user_key =
                                Some(Bytes::copy_from_slice(user.key()));
        ...
        let now = get_now_timestamp(context.time_provider().as_ref());
...
impl EncryptedWriter {
    ...
    pub fn with_identity(
        ...
        let mut buffer =
            BytesMut::with_capacity(nonce.len() + identity_keys.len() * 16);
        buffer.put(nonce);
        ...
        fn make_eih(
            ...
            match method {
                CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                    ...
                    let cipher =
                        Aes128::new_from_slice(enc_key).expect("AES-128");

                    let ipsk_plain_text =
                        <&Block as TryFrom<&[u8]>>::try_from(ipsk_plain_text)
                            .expect("ipsk length mismatch");
                ...
                CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
                    ...
                    let cipher =
                        Aes256::new_from_slice(enc_key).expect("AES-256");
        ...
        loop {
            match self.state {
                EncryptWriteState::AssembleHeader => {
                    ...
                    let header_len =
                        1 + 8 + request_salt_len + 2 + self.cipher.tag_len();
                    self.buffer.reserve(header_len);

                    let mbuf = &mut self.buffer.chunk_mut()[..header_len];
                    let mbuf = unsafe {
                        slice::from_raw_parts_mut(mbuf.as_mut_ptr(), mbuf.len())
                    };
