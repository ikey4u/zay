    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn handshake(&mut self) -> io::Result<Address> {
        ...
        let header =
            TcpRequestHeader::read_from(self.stream.method(), self).await?;
        ...
        if let TcpRequestHeader::Aead2022(ref header) = header {
            ...
            if header.padding_size == 0 {
                let (chunk_count, chunk_remaining) =
                    self.stream.current_data_chunk_remaining();
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    fn poll_read(
        ...
        if !self.has_handshaked {
            return Err(io::Error::other("stream is not handshaked yet"))
                .into();
        }
        ...
        #[cfg(feature = "aead-cipher-2022")]
        if let ProxyServerStreamWriteState::PrepareHeader(waker) =
            this.writer_state
