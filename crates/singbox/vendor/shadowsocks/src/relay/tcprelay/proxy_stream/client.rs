...
impl ProxyClientStream<OutboundTcpStream> {
    ...
    where
        A: Into<Address>,
    {
        Self::connect_with_opts(context, svr_cfg, addr, &DEFAULT_CONNECT_OPTS)
            .await
    }
        ...
        F: FnOnce(OutboundTcpStream) -> S,
    {
        let stream = match svr_cfg.timeout() {
            Some(d) => {
                match time::timeout(
                    d,
                    OutboundTcpStream::connect_server_with_opts(
                        &context,
    ...
    fn poll_read(
        ...
        loop {
            match this.reader_state {
                ...
                #[cfg(feature = "aead-cipher-2022")]
                ProxyClientStreamReadState::CheckRequestNonce => {
                    ready!(this.stream.as_mut().poll_read_decrypted(
                        cx,
                        this.context,
                        buf
                    ))?;
                    ...
                    let (data_chunk_count, _) =
                        this.stream.current_data_chunk_remaining();
        ...
        loop {
            match this.writer_state {
                &mut ProxyClientStreamWriteState::Connect(ref addr) => {
                    let buffer = make_first_packet_buffer(
                        this.stream.method(),
                        addr,
                        buf,
                    );
                    ...
                    *(this.writer_state) =
                        ProxyClientStreamWriteState::Connecting(buffer);
                }
                &mut ProxyClientStreamWriteState::Connecting(ref buffer) => {
                    let n =
                        ready!(this.stream.poll_write_encrypted(cx, buffer))?;
