        ...
        W: AsyncWrite + Unpin + ?Sized,
    {
        loop {
            ...
            while self.pos < self.cap {
                let me = &mut *self;
                let i = ready!(
                    writer.as_mut().poll_write(cx, &me.buf[me.pos..me.cap])
                )?;
                if i == 0 {
    ...
    B: AsyncRead + AsyncWrite + Unpin + ?Sized,
{
    ...
    fn poll_impl(
        ...
        let poll_a_to_b =
            transfer_one_direction(cx, a_to_b, a.as_mut(), b.as_mut())?;
        let poll_b_to_a =
            transfer_one_direction(cx, b_to_a, b.as_mut(), a.as_mut())?;
