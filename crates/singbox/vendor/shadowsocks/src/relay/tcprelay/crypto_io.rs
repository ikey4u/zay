...
use super::aead::{
    DecryptedReader as AeadDecryptedReader,
    EncryptedWriter as AeadEncryptedWriter,
};
#[cfg(feature = "aead-cipher-2022")]
use super::aead_2022::{
    DecryptedReader as Aead2022DecryptedReader,
    EncryptedWriter as Aead2022EncryptedWriter,
};
#[cfg(feature = "stream-cipher")]
use super::stream::{
    DecryptedReader as StreamDecryptedReader,
    EncryptedWriter as StreamEncryptedWriter,
};
...
impl DecryptedReader {
    ...
    pub fn with_user_manager(
        ...
        match method.category() {
            ...
            CipherCategory::Stream => {
                Self::Stream(StreamDecryptedReader::new(method, key))
            }
            #[cfg(feature = "aead-cipher")]
            CipherCategory::Aead => {
                Self::Aead(AeadDecryptedReader::new(method, key))
            }
            CipherCategory::None => {
                let _ = method;
                let _ = key;
            ...
            CipherCategory::Aead2022 => {
                Self::Aead2022(Aead2022DecryptedReader::with_user_manager(
                    stream_ty,
                    method,
                    key,
                    user_manager,
        ...
        S: AsyncRead + Unpin + ?Sized,
    {
        match *self {
            #[cfg(feature = "stream-cipher")]
            Self::Stream(ref mut reader) => reader
                .poll_read_decrypted(cx, context, stream, buf)
                .map_err(Into::into),
            #[cfg(feature = "aead-cipher")]
            Self::Aead(ref mut reader) => reader
                .poll_read_decrypted(cx, context, stream, buf)
                .map_err(Into::into),
            Self::None => {
                let _ = context;
            ...
            #[cfg(feature = "aead-cipher-2022")]
            Self::Aead2022(ref mut reader) => reader
                .poll_read_decrypted(cx, context, stream, buf)
                .map_err(Into::into),
        }
    }
...
impl EncryptedWriter {
    ...
    pub fn new(
        ...
        match method.category() {
            #[cfg(feature = "stream-cipher")]
            CipherCategory::Stream => {
                Self::Stream(StreamEncryptedWriter::new(method, key, nonce))
            }
            #[cfg(feature = "aead-cipher")]
            CipherCategory::Aead => {
                Self::Aead(AeadEncryptedWriter::new(method, key, nonce))
            }
            CipherCategory::None => {
                let _ = key;
                let _ = nonce;
            ...
            #[cfg(feature = "aead-cipher-2022")]
            CipherCategory::Aead2022 => {
                Self::Aead2022(Aead2022EncryptedWriter::new(
    ...
    pub fn with_identity(
        ...
        match method.category() {
            ...
            #[cfg(feature = "aead-cipher-2022")]
            CipherCategory::Aead2022 => {
                Self::Aead2022(Aead2022EncryptedWriter::with_identity(
                    context.time_provider(),
                    stream_ty,
                    method,
                    key,
        ...
        S: AsyncWrite + Unpin + ?Sized,
    {
        match *self {
            #[cfg(feature = "stream-cipher")]
            Self::Stream(ref mut writer) => writer
                .poll_write_encrypted(cx, stream, buf)
                .map_err(Into::into),
            #[cfg(feature = "aead-cipher")]
            Self::Aead(ref mut writer) => writer
                .poll_write_encrypted(cx, stream, buf)
                .map_err(Into::into),
            Self::None => {
            ...
            #[cfg(feature = "aead-cipher-2022")]
            Self::Aead2022(ref mut writer) => writer
                .poll_write_encrypted(cx, stream, buf)
                .map_err(Into::into),
        }
    }
    ...
    pub fn set_request_nonce(&mut self, request_nonce: Bytes) {
        match *self {
            ...
            Self::Aead2022(ref mut writer) => {
                writer.set_request_salt(request_nonce)
            }
            _ => {
                let _ = request_nonce;
    ...
    pub fn reset_cipher_with_key(&mut self, key: &[u8]) {
        match *self {
            ...
            _ => {
                let _ = key;
                panic!(
                    "only AEAD-2022 cipher could authenticate with multiple users"
                );
            }
        }
    }
...
impl<S> CryptoStream<S> {
    ...
    pub fn from_stream_with_identity(
        ...
        let iv = match category {
            ...
            CipherCategory::Stream => {
                ...
                trace!(
                    "generated Stream cipher IV {:?}",
                    ByteStr::new(&local_iv)
                );
                local_iv
            }
            #[cfg(feature = "aead-cipher")]
            CipherCategory::Aead => {
                ...
                trace!(
                    "generated AEAD cipher salt {:?}",
                    ByteStr::new(&local_salt)
                );
                local_salt
            }
            CipherCategory::None => {
        ...
        Self {
            stream,
            dec: DecryptedReader::with_user_manager(
                stream_ty,
                method,
                key,
                user_manager,
            ),
            enc: EncryptedWriter::with_identity(
                context,
                stream_ty,
                method,
                key,
    ...
    pub fn set_request_nonce(&mut self, request_nonce: &[u8]) {
        self.enc
            .set_request_nonce(Bytes::copy_from_slice(request_nonce))
    }
    ...
    pub(crate) fn current_data_chunk_remaining(&self) -> (u64, usize) {
        match self.dec {
            DecryptedReader::Aead2022(ref dec) => {
