    ...
    pub fn read_cursor<T: AsRef<[u8]>>(
        ...
        match atyp {
            consts::SOCKS5_ADDR_TYPE_IPV4 => {
                ...
                Ok(Self::SocketAddress(SocketAddr::V4(SocketAddrV4::new(
                    addr, port,
                ))))
            }
            ...
            consts::SOCKS5_ADDR_TYPE_DOMAIN_NAME => {
                ...
                let addr = String::from_utf8(buf)
                    .map_err(|_| Error::AddressDomainInvalidEncoding)?;
        ...
        match addr_type {
            ...
            consts::SOCKS5_ADDR_TYPE_IPV6 => {
                ...
                let bytes_buf = unsafe {
                    slice::from_raw_parts_mut(buf.as_mut_ptr() as *mut _, 18)
                };
...
impl Debug for Address {
    ...
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match *self {
            ...
            Self::DomainNameAddress(ref addr, ref port) => {
                write!(f, "{addr}:{port}")
            }
        }
    }
...
impl ToSocketAddrs for Address {
    ...
    fn to_socket_addrs(&self) -> io::Result<vec::IntoIter<SocketAddr>> {
        match self.clone() {
            ...
            Self::DomainNameAddress(addr, port) => {
            ...
            Err(..) => {
                ...
                match (sp.next(), sp.next()) {
                    (Some(dn), Some(port)) => match port.parse::<u16>() {
                        Ok(port) => {
                            Ok(Self::DomainNameAddress(dn.to_owned(), port))
                        }
