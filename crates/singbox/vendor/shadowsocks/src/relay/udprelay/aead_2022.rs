...
#[inline]
pub fn get_now_timestamp(context: &Context) -> u64 {
    match context
        .now()
    ...
    match method {
        ...
        | CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
            ...
            match method {
                CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                    let cipher =
                        Aes128::new_from_slice(ipsk).expect("AES-128 init");
                    let block = <&mut Block as TryFrom<&mut [u8]>>::try_from(
                        packet_header,
                    )
                ...
                CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
                    let cipher =
                        Aes256::new_from_slice(ipsk).expect("AES-256 init");
                    let block = <&mut Block as TryFrom<&mut [u8]>>::try_from(
                        packet_header,
                    )
                    .expect("Packet header length mismatch");
    ...
    match method {
        CipherKind::AEAD2022_BLAKE3_CHACHA20_POLY1305 => {
            ...
            let session_id = {
                ...
                let session_id_slice: &[u64] = unsafe {
                    slice::from_raw_parts(
                        session_id_buf.as_ptr() as *const _,
                        1,
                    )
                };
        ...
        | CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
            ...
            match method {
                CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                    let cipher =
                        Aes128::new_from_slice(key).expect("AES-128 init");
                    let block = <&mut Block as TryFrom<&mut [u8]>>::try_from(
                        packet_header,
                    )
                    .expect("Packet header length mismatch");
                ...
                CipherKind::AEAD2022_BLAKE3_AES_256_GCM => {
                    let cipher =
                        Aes256::new_from_slice(key).expect("AES-256 init");
                    let block = <&mut Block as TryFrom<&mut [u8]>>::try_from(
                        packet_header,
                    )
                    .expect("Packet header length mismatch");
                    ...
                    match method {
                        CipherKind::AEAD2022_BLAKE3_AES_128_GCM => {
                            ...
                            cipher.decrypt_block(
                                <&mut Block as TryFrom<&mut [u8]>>::try_from(
                                    eih,
                                )
...
fn get_nonce_len(method: CipherKind) -> usize {
    match method {
        CipherKind::AEAD2022_BLAKE3_AES_128_GCM
        | CipherKind::AEAD2022_BLAKE3_AES_256_GCM => 0,
...
pub fn encrypt_client_payload_aead_2022(
    ...
    dst.put_u64(get_now_timestamp(context));
...
pub fn decrypt_client_payload_aead_2022(
    ...
    let now = get_now_timestamp(context);
