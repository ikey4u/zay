...
use std::os::windows::io::{
    AsRawSocket, AsSocket, BorrowedSocket, IntoRawSocket, RawSocket,
};
use std::{
    io::{self, ErrorKind},
    net::SocketAddr,
...
use super::{
    compat::{
        DatagramReceive, DatagramReceiveExt, DatagramSend, DatagramSendExt,
        DatagramSocket,
    },
    crypto_io::{
        ProtocolError, ProtocolResult, decrypt_client_payload,
        decrypt_server_payload, encrypt_client_payload, encrypt_server_payload,
    },
};

static DEFAULT_CONNECT_OPTS: LazyLock<ConnectOpts> =
    LazyLock::new(Default::default);
static DEFAULT_SOCKET_CONTROL: LazyLock<UdpSocketControlData> =
    LazyLock::new(UdpSocketControlData::default);
...
impl ProxySocket<ShadowUdpSocket> {
    ...
    pub async fn connect_with_opts(
        ...
        let socket = ShadowUdpSocket::connect_server_with_opts(
            &context,
    ...
    pub async fn bind_with_opts(
        ...
        let socket = match svr_cfg.udp_external_addr() {
            ServerAddr::SocketAddr(sa) => {
                ShadowUdpSocket::listen_with_opts(sa, opts).await?
            }
    ...
    S: DatagramSend,
{
    ...
    pub async fn send_with_ctrl(
        ...
        let send_len = match self.send_timeout {
            ...
            Some(d) => match time::timeout(d, self.io.send(&send_buf)).await {
                ...
                Err(..) => {
                    return Err(io::Error::from(ErrorKind::TimedOut).into());
                }
            },
        };
        ...
        match self.io.poll_send(cx, &send_buf).map_err(|x| x.into()) {
            Poll::Ready(Ok(l)) => {
                if l == n_send_buf {
                    ...
                    Poll::Ready(Err(
                        io::Error::from(ErrorKind::WriteZero).into()
                    ))
                }
            }
            x => x,
    ...
    pub fn poll_send_to_with_ctrl(
        ...
        match self
            .io
            .poll_send_to(cx, &send_buf, target)
            .map_err(|x| x.into())
        {
            Poll::Ready(Ok(l)) => {
                if l == n_send_buf {
    ...
    pub async fn send_to_with_ctrl(
        ...
        let send_len = match self.send_timeout {
            ...
            Some(d) => {
                match time::timeout(d, self.io.send_to(&send_buf, target)).await
                {
                    Ok(Ok(l)) => l,
                    Ok(Err(err)) => return Err(err.into()),
                    Err(..) => {
    ...
    S: DatagramReceive,
{
    ...
    ) -> ProxySocketResult<(usize, Address, usize, Option<UdpSocketControlData>)>
    {
        ...
        let (n, addr, control) = match self.decrypt_recv_buffer(
            &mut recv_buf[..recv_n],
    ...
    pub async fn recv_from_with_ctrl(
        ...
        let (recv_n, target_addr) = match self.recv_timeout {
            ...
            Some(d) => {
                match time::timeout(d, self.io.recv_from(recv_buf)).await {
                    Ok(Ok(l)) => l,
                    Ok(Err(err)) => return Err(err.into()),
                    Err(..) => {
                        return Err(io::Error::from(ErrorKind::TimedOut).into());
        ...
        let (n, addr, control) = match self.decrypt_recv_buffer(
            ...
            Ok(x) => x,
            Err(err) => {
                return Err(ProxySocketError::ProtocolErrorWithPeer(
                    target_addr,
                    err,
                ));
            }
    ...
    > {
        ...
        match self.decrypt_recv_buffer(
            recv_buf.filled_mut(),
            self.user_manager.as_deref(),
        ) {
