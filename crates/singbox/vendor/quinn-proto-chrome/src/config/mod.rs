...
pub struct ClientConfig {
    ...
    /// Emit the client-side QUIC wire shape used by Chrome.
...
impl ClientConfig {
    ...
    /// Enable the Chrome client QUIC wire profile.
    ///
    /// This is intended for one-connection-per-socket clients using a
    /// zero-length local connection ID generator.
    pub fn chrome_parrot(&mut self, enabled: bool) -> &mut Self {
        self.chrome_parrot = enabled;
        self
    }
