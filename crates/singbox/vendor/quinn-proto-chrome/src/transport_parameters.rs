...
macro_rules! make_struct {
    {$($(#[$doc:meta])* $name:ident ($id:ident) = $default:expr,)*} => {
        ...
        pub struct TransportParameters {
            ...
            /// Encode this client parameter set with Chrome's parameter set,
            /// ordering, private option and GREASE shape.
            pub(crate) chrome_parrot: bool,
        }
...
impl TransportParameters {
    pub(crate) fn enable_chrome_parrot(&mut self) {
        self.chrome_parrot = true;
        self.max_udp_payload_size = VarInt::from_u32(1_472);
        self.max_datagram_frame_size = Some(VarInt::from_u32(65_536));
        self.ack_delay_exponent = VarInt::from_u32(3);
        self.max_ack_delay = VarInt::from_u32(25);
        self.disable_active_migration = false;
        self.active_connection_id_limit = VarInt::from_u32(2);
        self.min_ack_delay = None;
    }
...
impl TransportParameters {
    ...
    fn write_chrome<W: BufMut>(&self, w: &mut W) {
        const VERSION_INFORMATION: u64 = 0x11;
        const GOOGLE_CONNECTION_OPTIONS: u64 = 0x3128;
        const CHROME_GREASE_MAX_VALUE_LEN: usize = 15;

        fn varint_parameter(id: u64, value: VarInt) -> Vec<u8> {
            let mut encoded = Vec::new();
            encoded.write_var(id);
            encoded.write_var(value.size() as u64);
            encoded.write(value);
            encoded
        }
        ...
        let mut initial_source = Vec::new();
        initial_source
            .write_var(TransportParameterId::InitialSourceConnectionId as u64);
        let initial_source_cid = self
            .initial_src_cid
            .unwrap_or_else(|| ConnectionId::new(&[]));
        initial_source.write_var(initial_source_cid.len() as u64);
        initial_source.put_slice(&initial_source_cid);
        parameters.push(initial_source);

        let mut rng = rand::rng();
        let mut version_information = Vec::new();
        version_information.write_var(VERSION_INFORMATION);
        version_information.write_var(12);
        version_information.put_u32(1);
        let mut available = [1_u32, 0_u32];
        let mut grease = [0_u8; 4];
        rng.fill(&mut grease);
        for byte in &mut grease {
            *byte = *byte & 0xf0 | 0x0a;
        }
        available[1] = u32::from_be_bytes(grease);
        if rng.random::<bool>() {
            available.swap(0, 1);
        }
        for version in available {
            version_information.put_u32(version);
        }
        parameters.push(version_information);

        if let Some(value) = self.max_datagram_frame_size {
            parameters.push(varint_parameter(
                TransportParameterId::MaxDatagramFrameSize as u64,
                value,
            ));
        }

        let mut google_options = Vec::new();
        google_options.write_var(GOOGLE_CONNECTION_OPTIONS);
        google_options.write_var(4);
        google_options.put_slice(b"ORIG");
        parameters.push(google_options);

        let n_min = ((1_u64 << 30) - 27).div_ceil(31);
        let n_max = ((1_u64 << 62) - 1 - 27) / 31;
        let reserved_id = 31 * rng.random_range(n_min..n_max) + 27;
        let value_len = rng.random_range(0..=CHROME_GREASE_MAX_VALUE_LEN);
        let mut reserved = Vec::new();
        reserved.write_var(reserved_id);
        reserved.write_var(value_len as u64);
        let mut value = vec![0_u8; value_len];
        rng.fill(value.as_mut_slice());
        reserved.put_slice(&value);
        parameters.push(reserved);

        parameters.shuffle(&mut rng);
        for parameter in parameters {
            w.put_slice(&parameter);
        }
    }
...
mod test {
    ...
    #[test]
    fn chrome_parrot_parameter_set_matches_chrome_wire_contract() {
        use std::collections::BTreeMap;

        let mut params = TransportParameters {
            max_idle_timeout: VarInt::from_u32(30_000),
            max_udp_payload_size: VarInt::from_u32(1_472),
            initial_max_data: VarInt::from_u32(15_728_640),
            initial_max_stream_data_bidi_local: VarInt::from_u32(6_291_456),
            initial_max_stream_data_bidi_remote: VarInt::from_u32(6_291_456),
            initial_max_stream_data_uni: VarInt::from_u32(6_291_456),
            initial_max_streams_bidi: VarInt::from_u32(100),
            initial_max_streams_uni: VarInt::from_u32(103),
            initial_src_cid: Some(ConnectionId::new(&[])),
            ..TransportParameters::default()
        };
        params.enable_chrome_parrot();
        ...
        while input.has_remaining() {
            ...
            let length = usize::try_from(input.get_var().unwrap()).unwrap();
            let mut value = vec![0_u8; length];
            input.copy_to_slice(&mut value);
            assert!(decoded.insert(id, value).is_none(), "duplicate {id:#x}");
        }

        let expected = [
            0x01_u64, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
            0x0f, 0x11, 0x20, 0x3128,
        ];
        assert_eq!(decoded.len(), expected.len() + 1);
        for id in expected {
            assert!(decoded.contains_key(&id), "missing {id:#x}");
        }
        assert_eq!(decoded[&0x0f], b"");
        assert_eq!(decoded[&0x3128], b"ORIG");
        assert_eq!(decoded[&0x11].len(), 12);
        assert_eq!(&decoded[&0x11][..4], 1_u32.to_be_bytes());
        assert!(!decoded.contains_key(&0x0a));
        assert!(!decoded.contains_key(&0x0b));
        assert!(!decoded.contains_key(&0x0c));
        assert!(!decoded.contains_key(&0x0e));
        assert!(!decoded.contains_key(&0x2ab2));
        assert!(!decoded.contains_key(&0xff04de1b));

        let reserved = decoded
            .keys()
            .copied()
            .find(|id| !expected.contains(id))
            .unwrap();
        assert_eq!(reserved % 31, 27);
        assert!(reserved >= 1 << 30);
        assert!(decoded[&reserved].len() <= 15);
    }
