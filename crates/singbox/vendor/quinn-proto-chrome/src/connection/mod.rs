...
pub struct Connection {
    ...
    spin_enabled: bool,
    /// Emit Chromium's client-side handshake packet layout.
    chrome_parrot: bool,
...
impl Connection {
    ...
    pub(crate) fn enable_chrome_parrot(&mut self) {
        // Chrome starts its Initial packet number space at 1. This is applied
        // before the endpoint exposes the connection or polls a transmit.
    ...
    pub fn poll_transmit(
        ...
        let chrome_client = self.chrome_parrot && self.side.is_client();
        ...
        while space_idx < spaces.len() {
            ...
            if chrome_client
                ...
                && !self.spaces[space_id].pending.crypto.is_empty()
                && !self.spaces[space_id].ping_pending
                && !self.spaces[space_id].immediate_ack_pending
            {
                ack_eliciting = false;
            ...
            // Chromium sends one encryption-level packet per UDP datagram while
            // handshaking, including consecutive Initial packets.
            if chrome_client {
                coalesce = false;
            }
    ...
    fn on_ack_received(
        ...
        for packet in newly_acked.elts() {
            if let Some(info) = self.spaces[space].take(packet) {
                ...
                self.on_packet_acked(now, Some((space, packet)), info);
            }
        }
        ...
        self.path.congestion.on_end_congestion_event(
            now,
            self.path.in_flight.bytes,
            self.app_limited,
            self.spaces[space].largest_acked_packet,
    ...
    fn on_packet_acked(
        &mut self,
        now: Instant,
        packet_number: Option<(SpaceId, u64)>,
        info: SentPacket,
    ) {
        ...
        if info.ack_eliciting && self.path.challenge.is_none() {
            ...
            if let Some((packet_space, packet_number)) = packet_number {
    ...
    fn on_loss_detection_timeout(&mut self, now: Instant) {
        if let Some((_, pn_space)) = self.loss_time_and_space() {
            ...
            self.path.congestion.on_end_congestion_event(
                ...
                self.path.in_flight.bytes,
                self.app_limited,
                self.spaces[pn_space].largest_acked_packet,
                &self.path.rtt,
            );
            self.set_loss_detection_timer(now);
            return;
    ...
    fn detect_lost_packets(&mut self, now: Instant, pn_space: SpaceId, due_to_ack: bool) {
        ...
        if let Some(largest_lost) = lost_packets.last().cloned() {
            ...
            for &packet in &lost_packets {
                ...
                if info.ack_eliciting && self.path.challenge.is_none() {
                    self.path.congestion.on_lost_packet(
                        now,
                        info.time_sent,
                        info.size.into(),
                        pn_space as u8,
        ...
        match packet.header {
            Header::Retry {
                ...
                if let Some(info) = space.take(0) {
                    self.on_packet_acked(now, None, info);
                };
    ...
    fn populate_packet(
        ...
        // Chrome sends the Handshake ACK in its own packet and leaves the
        // client's Finished CRYPTO data for the following datagram.
        if chrome_client
            && space_id == SpaceId::Handshake
            && sent.largest_acked.is_some()
            && !space.pending.crypto.is_empty()
        {
            return sent;
        }
        ...
        while buf.len() + frame::Crypto::SIZE_BOUND < max_size && !is_0rtt {
            ...
            // When the ClientHello spans multiple Initial packets, Chromium
            // sends its beginning and end first, leaving the middle queued.
            if chrome_client && space_id == SpaceId::Initial && frame.offset == 0 {
                let available = max_size - buf.len();
                if let Some((head_len, tail_len)) = chrome_chaos::crypto_split(
                    frame.data.len(),
                    frame.offset,
                    available,
                    &mut self.rng,
                ) {
                    let tail_start = frame.data.len() - tail_len;
                    let head = frame::Crypto {
                        offset: frame.offset,
                        data: frame.data.slice(..head_len),
                    };
                    let tail = frame::Crypto {
                        offset: frame.offset + tail_start as u64,
                        data: frame.data.slice(tail_start..),
                    };
                    if buf.len()
                        + 1
                        + VarInt::size(unsafe { VarInt::from_u64_unchecked(head.offset) })
                        + VarInt::size(unsafe {
                            VarInt::from_u64_unchecked(head.data.len() as u64)
                        })
                        + head.data.len()
                        + 1
                        + VarInt::size(unsafe { VarInt::from_u64_unchecked(tail.offset) })
                        + VarInt::size(unsafe {
                            VarInt::from_u64_unchecked(tail.data.len() as u64)
                        })
                        + tail.data.len()
                        <= max_size
                    {
                        head.encode(buf);
                        tail.encode(buf);
                        self.stats.frame_tx.crypto += 2;
                        let retransmits = sent.retransmits.get_or_create();
                        retransmits.crypto.push_back(head);
                        retransmits.crypto.push_back(tail);
                        if head_len < tail_start {
                            space.pending.crypto.push_front(frame::Crypto {
                                offset: frame.offset + head_len as u64,
                                data: frame.data.slice(head_len..tail_start),
                            });
                        }
                        break;
                    }
