...
use super::{Connection, SentFrames, chrome_chaos, spaces::SentPacket};
use crate::{
...
impl PacketBuilder {
    ...
    pub(super) fn finish_and_track(
        ...
        let bytes_in_flight = conn.path.in_flight.bytes;
        conn.path
            .sent(exact_number, packet, &mut conn.spaces[space_id]);
        if size != 0 && ack_eliciting {
            conn.path.congestion.on_sent_packet(
                now,
                u64::from(size),
                space_id as u8,
                exact_number,
    ...
    pub(super) fn finish(
        ...
        if conn.chrome_parrot && conn.side.is_client() && self.space == SpaceId::Initial {
            chrome_chaos::protect_initial_payload(
                buffer,
                self.partial_encode.start + self.partial_encode.header_len,
                &mut conn.rng,
            );
        }
