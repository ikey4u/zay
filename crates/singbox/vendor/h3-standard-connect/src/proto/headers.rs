...
impl Pseudo {
    fn request(method: Method, uri: Uri, ext: Extensions) -> Self {
        ...
        // A standard CONNECT request only carries :method and :authority.
        // :scheme and :path are reserved for extended CONNECT requests.
        // See RFC 9114 section 4.4 and hyperium/h3#322.
...
mod tests {
    ...
    #[test]
    fn standard_connect_omits_scheme_and_path() {
        let pseudo = Pseudo::request(
            Method::CONNECT,
            "https://proxy.example/".parse().unwrap(),
            Extensions::new(),
        );

        assert!(pseudo.scheme.is_none());
        assert!(pseudo.path.is_none());
        assert_eq!(pseudo.authority.unwrap().as_str(), "proxy.example");
    }

    #[test]
    fn extended_connect_retains_scheme_and_path() {
        let mut extensions = Extensions::new();
        extensions.insert(Protocol::CONNECT_UDP);
        let pseudo = Pseudo::request(
            Method::CONNECT,
        ...
        assert_eq!(pseudo.scheme.unwrap(), Scheme::HTTPS);
        assert_eq!(
            pseudo.path.unwrap().as_str(),
