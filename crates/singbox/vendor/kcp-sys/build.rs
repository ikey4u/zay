...
fn clang_target(target: &str) -> String {
    if target == "aarch64-apple-ios-sim" {
        let minimum = env::var("IPHONEOS_DEPLOYMENT_TARGET")
            .unwrap_or_else(|_| "16.0".to_owned());
        return format!("arm64-apple-ios{minimum}-simulator");
    }
    if target == "x86_64-apple-ios" {
        let minimum = env::var("IPHONEOS_DEPLOYMENT_TARGET")
            .unwrap_or_else(|_| "16.0".to_owned());
        return format!("x86_64-apple-ios{minimum}-simulator");
    }
    let mut parts = target.split('-');
    let arch = clang_arch(parts.next().unwrap_or(target));
    let rest = parts.collect::<Vec<_>>();
