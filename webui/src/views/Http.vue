...
const form = ref({ root: '.', listen: '127.0.0.1:8080', spa: false, cors: false })

async function refresh() {
  const all = await api.listJobs()
  jobs.value = all.filter((j) => j.kind === 'http')
}
...
<template>
  <div class="space-y-6">
    <h1 class="text-2xl font-semibold">HTTP static server</h1>
    <div
      v-if="error"
    ...
    <section class="rounded-xl border border-zinc-800 bg-zinc-900/50 p-4">
      ...
      <div class="mt-3 grid gap-3">
        ...
        <label class="flex items-center gap-2 text-sm text-zinc-300"
          ><input v-model="form.spa" type="checkbox">
          SPA fallback</label
        >
        <label class="flex items-center gap-2 text-sm text-zinc-300"
          ><input v-model="form.cors" type="checkbox">
          CORS</label
        >
      </div>
      <button
